...
| **Founder/governance cluster** | CDL-003, 004, 005, 006, 008, 009, 010, 015 | GOV-A or GOV-C (partial) | CDL-006 challenge-node spec/runtime helper complete through Phase 1382; CDL-009 fork-legitimacy UX helper complete through Phase 1383; public governance activation remains gated; CDL-005 preconditions CDL-025–031 which are all NONE |
...
| CDL-057 | Epoch-boundary witness lane | **PARTIAL** | `ilc_core/epoch/epoch_boundary_witness_runtime.py` | `CDL_057_DEPENDENCY = "cdl_057_ratified_511.v0.1"`. Witness records plus Phase 1364 active blocking flag. `EPOCH_BOUNDARY_WITNESS_RUNTIME_VERSION = "epoch_boundary_witness_blocking_active_phase_1364.v0.1"`; `BLOCKING_AUTHORITY_DEFERRED = False`; `is_blocking_authority_active()` derives from that flag. Production infrastructure activation remains separately gated. |
...
| CDL-009 | Fork legitimacy/user signaling (`signature-badge+eligibility rules`) | Ph 993 | **GOV-C** | A: `ilc_core/network/d2d/bootstrap_fetch_runtime.py` ML-DSA-65 signature verification is the "signature-badge" enforcement; CDL-073 homoiconic bootstrap + Genesis lineage integrity is the "eligibility rules" expression. Phase 1383 adds `ilc_core/governance/fork_legitimacy_runtime.py`, a public-RC-excluded CLI/operator fork-signal inspection helper with badge schema validation, eligibility rules, and deterministic inspection results. B: Public API activation, graph writes, and governance execution remain separately gated. |
...
| ADR-0028 | Settlement substrate graduation and governance route (Option B: Mysticeti) | **PARTIAL** | `ilc_consensus/` (Rust crate — M-series complete); `docs/specs/ilc_security_review_scope_1384_v0.1.md`; `docs/specs/ilc_tla_plus_safetynodualcert_disposition_1385_v0.1.md` | M-001–M-022 complete. `ilc_consensus/` contains full ILC-native DAG-BFT crate (BLS12-381, QUIC, LMDB). 4-validator M-009 testnet config exists. HIGH-002 fixed (Phase 842). CDL-067 ratified (Phase 709). Option B selected (Phase 814). Gap: production connectivity/key-ceremony/hardening gates remain open. HIGH-001 log-layer defense exists from Phase 1359 but non-loopback verification and sender-privacy non-claim boundaries remain. Phase 1384 records AI-assisted/community security review scope; commercial audit firm engagement is not required, but Phase 1387 requires project-authority disposition for every known HIGH finding. Phase 1385 records `safetynodualcert_deferred_with_authority_phase_1385`: owned-object Spec B has bounded TLC evidence, while epoch-checkpoint/shared-object dual-cert safety remains deferred to Spec D or equivalent future formal-methods work. |
