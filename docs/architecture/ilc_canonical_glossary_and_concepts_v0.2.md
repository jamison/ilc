...
| :--- | :--- | :--- |
| **AtlasSliceManifest** | The canonical "star map" object going forward for install/build packaging: a signed, content-addressed, verifiable manifest for a specific graph projection slice. It commits to the slice identity, source LMDB root, projection filter, included-node Merkle root, cross-section reference count, exclusion policy, privacy budget, installer profile, and later receipt/signature fields. CDL-098 authorizes Genesis to sign public-section AtlasSliceManifest records only for `genesis_core_star_map`, `public_protocol_graph`, and `support_candidate_graph`; `excluded_private_material` remains owner-signed or private-policy governed. The AtlasSliceManifest is the **portable package truth**: verifiable by any recipient without access to the originating LMDB. Long-term, the canonical authority commitment is the native `__signed_slices__` LMDB record; the manifest is the portable distribution witness derived from it (see §2.13). | Use `AtlasSliceManifest` when referring to the signed executable map or install/hydration guide for a graph slice. Do not use it to mean ADR-0003 route indexes, ADR-0033 published navigation nodes, raw graph authority, or an LMDB database itself. Do not conflate "portable package truth" (manifest) with "canonical authority commitment" (native LMDB record). |
...
### 2.13 Block 6 / Post-RC Signing and Economics Terms (Phase 1575c-Fix3a addendum, 2026-07-16)

These terms are load-bearing in the Block 6 signing pipeline (Fix3a, Fix3, Fix4),
the Window 1576+ AtlasSliceManifest production lanes, and the post-RC economics
surface. They are canonical for operational use but not yet promoted through the
full §1.2 promotion path.

#### 2.13.1 Signing and Graph Packaging

| Term | Definition | Canon Rule |
| :--- | :--- | :--- |
| **Core Slice 0** | The Genesis authority signing kernel: the curated set of foundational nodes (58 nodes post-Fix3a: 57 from `genesis_core_star_map_v0.4.json` plus CDL-098) signed by the Genesis operator under ML-DSA-65 to establish the authority root for all downstream signed artifacts. Distinct from the full Public-RC Baseline. | Use `Core Slice 0` for the authority signing kernel specifically. Do not use it to mean the full 1,035-node Public-RC Baseline, the LMDB row-store, or the `genesis_core_star_map` graph projection label. |
| **Public-RC Baseline (Slice 1)** | The 1,035-node, 3,841-edge Atlas graph package produced by Phase 1575c-Fix2 under the `genesis_core_star_map` + `public_protocol_graph` projection. It is signed after Core Slice 0 and references the Core Slice 0 digest as its authority root. Also called `genesis_v05_public_rc_baseline_slice_1`. | Use `Public-RC Baseline` or `Slice 1` for the 1,035-node package. Do not conflate with Core Slice 0 (the authority kernel) or with the full Atlas LMDB (which contains private/excluded material). |
| **Canonical authority commitment** | The native signed-slice record stored in the `__signed_slices__` LMDB table: Merkle root over a named slice projection's sorted logical content, ML-DSA signature, projection definition, and signing parameters. This is the primary authority object. The AtlasSliceManifest is derived from it. Long-range design target; the current Fix3/Fix3a JSON-extraction approach is the interim bridge to public RC. | Use `canonical authority commitment` for the LMDB-native signed record. Do not use it to mean the JSON AtlasSliceManifest, which is the portable distribution witness derived from it. |
| **Portable package truth** | What an AtlasSliceManifest provides to any external recipient: a self-contained, verifiable commitment to a slice's contents that can be checked without access to the originating LMDB. The manifest is the portable package truth; the native `__signed_slices__` record is the canonical authority commitment. | Use `portable package truth` to describe the AtlasSliceManifest's role for distribution/install. Do not use it to mean the primary authority object — that is the native LMDB record. |
| **Named slice projection** | A deterministic, versioned query over LMDB content that defines the exact set of nodes and edges to be signed (e.g., `WHERE graph_projection = 'genesis_core_star_map'`). The signing surface for a `__signed_slices__` record is always a named slice projection — never the whole LMDB. The whole local LMDB contains public slices, private overlays, pending local nodes, cache tables, indexes, and operational state; only the declared projection is signed. | Use `named slice projection` when describing what a signing ceremony covers. Explicitly reject "whole-LMDB signing" — LMDB B-tree page layout is not a stable canonical hash surface; logical projection content in sorted order is. |
| **Stub node** | A graph node with no `rationale`, no `node_kind`, no `layer`, and no `evidence` records. Stub nodes cannot be included in a signing ceremony without undermining the integrity of the signed kernel. Three stub nodes existed in `genesis_core_star_map_v0.4.json` (`adr:0009_bundle_distribution`, `cdl:096_werner_global_tier_authority`, `cdl:097_type_definition_node_authority`) and were fully enriched in Phase 1575c-Fix3a before signing. | Use `stub node` to flag nodes that block signing due to missing mandatory fields. A stub node is not merely a node with minor metadata gaps — it specifically lacks the four structural fields that establish its rationale for inclusion. |

#### 2.13.2 Economics and Credit

| Term | Definition | Canon Rule |
| :--- | :--- | :--- |
| **local_productive_credit (CDL-053)** | The CDL-053 Werner credit surface: an internal eligibility record tracking an agent's deployment work within a shard or local scope. It is an accounting signal internal to the protocol, not a separate token and not a form of ECU. CDL-053 is default-off and gated on research outcomes; it does not create a third coin or a parallel settlement path. The Werner credit architecture (§2.11) describes the theoretical framing; `local_productive_credit` is the specific CDL-053 runtime field. | Use `local_productive_credit` for the CDL-053 field only. Never describe it as a token, a coin, or an equivalent of ECU. If someone asks "is this a third coin?" the answer is: no — it is an internal eligibility record that may eventually influence ECU issuance capacity if CDL-053 is activated through its research gate. |
| **Organic exit** | The protocol-driven departure of an agent from active participation when its reputation score decays below the CDL-043 `ecu_score_floor` (0.5). Organic exit is a graph-mechanics outcome — the agent is not administratively removed; it ceases to meet eligibility thresholds for active roles. Distinct from administrative ejection. | Use `organic exit` for reputation-decay-driven departure. Do not conflate with ejection, banning, or key revocation. The agent identity persists in the graph; it simply fails eligibility checks until scores recover or the agent is pruned by CDL-043 pruning rules. |
| **Administrative ejection** | Removal of an agent from an active role through a quorum-authorized governance action (CDL process). Unlike organic exit, administrative ejection requires an explicit governance decision. No single authority — including Genesis — may administratively eject an agent unilaterally without quorum authorization. | Use `administrative ejection` for governance-action-driven removal. Do not use it to mean organic exit (reputation decay) or key revocation (a separate identity operation). |

#### 2.13.3 Privacy and Spectral Protocol

| Term | Definition | Canon Rule |
| :--- | :--- | :--- |
| **CCSS-SPECTRAL-01** | The cryptographic construction specification for privacy-preserving spectral neighborhood signaling, ratified in Block 6. Establishes the `SpectralRouteToken` as the successor mechanism to the failed jiggle-factor (σ-noise) approach. Key principle: eigenvalue arrays and `noise_sigma` must not appear in any Layer 3 gossip envelope. CDL-SIGMA-01 records the structural failure of the prior approach. | Use `CCSS-SPECTRAL-01` as the ratified specification authority for spectral route tokens. Do not use `noise_sigma` or transmit eigenvalue arrays at the wire level — these are Layer 3 forbidden fields. |
| **SpectralRouteToken** | The CCSS-SPECTRAL-01 route primitive: a capability-style token carrying a hiding commitment over the agent's local spectral value (`hiding_commitment(λ_local)`), a `route_purpose` field, sender ephemeral public key, message nonce, and epoch root. The commitment hides the raw λ_local value from relays; the salt is not relay-visible. Replaces the structurally broken σ-noise (jiggle factor) mechanism. | Use `SpectralRouteToken` for the CCSS-SPECTRAL-01 token. Verify: the token carries a *commitment* to λ_local, not the raw value. The hiding commitment scheme and salt lifecycle are specified in the full CCSS-SPECTRAL-01 spec (Window 1576+ target). |
