...
| **ECU (Epistemic Credit Unit)** | The protocol-internal credit unit. Constructed by verified work flowing through the review lane. Cannot be created from private-visibility or operator-local material. Not an external token. | Use `ECU` in all protocol, ledger, and economic surface references. Never use `ILC` to mean the internal credit unit. |
...
| **T0 / T0.5 / T1+ / T2 / T5 / T6** | The node lifecycle taxonomy (ADR-0041, Phase 1393 J-003). T0 = pending public admission quarantine; T0.5 = submitted, under review lane; T1+ = admitted objective node (reward-bearing); T2 = subjective/aesthetic; T5 = refutation; T6 = provenance/meta/activation. | Use the T-tier designation when referring to a node's admission status in the review lane. |
| **Review lane** | The jury-based admission process for public reward-bearing nodes. Defined by ADR-0043, implemented Phase 1415-1417. T0.5 nodes advance to T1+ via a VRF-assigned jury panel. | Use `review lane` for the admission mechanism; `review lane wiring` for its runtime implementation state. |
