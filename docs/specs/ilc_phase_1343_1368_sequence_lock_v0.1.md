...
| 1360 Fix1 | Four-validator epoch finalization diagnostic | BLOCKED_WITH_FINDINGS; records `phase_1360_fix1_epoch_finalization_still_blocked`; startup reproduced, UFW/macOS firewall ruled out as primary blockers, VPS accept path still closes before receive/dispatch, and no finalization proof token was recorded. |
| 1360 Fix2 | Four-validator epoch finalization follow-up | COMPLETE; NON-SENSITIVE; records `phase_1360_fix2_four_validator_epoch_finalization_proven` as injected checkpoint/local commit proof, not durable peer-to-peer BFT round; validator 1 reassigned to ilc-node-2; macOS inbound QUIC issue consistent with Tailscale/QUIC routing, exact cause unconfirmed without packet capture. |
...
| 1362 | Blocking-authority vehicle opening | SENSITIVE; Phase 1344 rerouted this away from CDL-053; future explicit GO and CDL mutation authority required; next planned phase. |
