...
| Voice sidecar CDL — authenticated pseudonymous audio relay | New CDL (unopened; no number assigned) | Opus 20ms frames over QUIC; ILC `agent_id` pseudonymous session auth (no central server); single or two-hop relay via CDL-078 L5 relay pool; relay earns CDL-078 credit for audio traffic; separate traffic-type classification from graph-content relay may require CDL-078 amendment or sibling CDL. Research note: `docs/research/ilc_voice_sidecar_pseudonymous_audio_relay_v0.1.md`. HIGH-001 mixing is incompatible with real-time audio (deliberate delay destroys voice); the L5 relay hop provides IP indirection only, not mixing anonymity. | Prerequisites: Public RC (Phase 1389), CDL-078 L5 relay live, sidecar manifest/SDK stable, CDL-088 identity activated, HIGH-001 landed (Phase 1359). Phase assignment Window 1399+. |
