...
| 1407 | CDL-093 maintenance lottery pool — deliberation and prelock | CDL prelock | NON-SENSITIVE | COMPLETE (multi-fix): base `9be7b2b8` (deliberation + CDL-093 prelock); Fix0 `b5c638bc`/`36d3b698`/`a4e0856d` (CDL-053 Werner credit opening); Fix1 `d302544b`/`fa7e3ce1` (CDL-053 prelock); Fix2 `aa8e7e9a`/`f654bacc`/`40579739` (CDL-053 ratification — SENSITIVE); Fix3 `33cf09da`/`3913ed13` (CDL-093 prelock amendment to Werner source + Q2 SIM). CDL-053 ratified, CDL-093 prelocked. Tokens: `cdl_053_ratified_phase_1407_fix2`, `cdl_093_prelock_amended_phase_1407_fix3`. |
| 1408 | CDL-093 maintenance lottery pool — ratification | CDL ratification | SENSITIVE | COMPLETE: C1 `851e8e8b` (evidence + tests); C2 `86e84c9f` (CDL mutation); backfill `9be7b2b8`. CDL-093 ratified. Token: `cdl_093_ratified_phase_1408`. Resolves `MAINTENANCE_LOTTERY_CDL_RATIFIED`. |
| 1409 | Maintenance lottery pool runtime stub | Runtime | NON-SENSITIVE | COMPLETE: `29bcf0c8`. `lottery_pool_runtime.py` stub; `LOTTERY_NOT_ACTIVATED=True`. Token: `maintenance_lottery_runtime_stub_committed_phase_1409`. |
| 1410 | VRF proof verifier specification and ADR | Spec / ADR | NON-SENSITIVE | COMPLETE: `0e12e7d0` (ADR-0042). Fix1 `0b484aa0` (pre-VRF hardening — 6 audit findings, PyNaCl binding gap note). ADR-0042 committed. Token: `adr_0042_vrf_proof_verifier_spec_committed_phase_1410`. |
| 1411 | VRF proof verifier implementation | Runtime | NON-SENSITIVE | COMPLETE: `e995cd8e` (implementation); `4410b111` (5 test gap fixes); `892ac960` (algebraic completeness proof comment in `_point_add`). 22 tests passing; RFC 9381 Appendix B.4 all 3 vectors verified. Token: `vrf_proof_verifier_implemented_phase_1411`. Resolves `VRF_VERIFIER_IMPLEMENTED` (partial — Phase 1412 integration + Phase 1413 tests needed for full MET). |
| 1412 | VRF integration with `jury_assignment_runtime.py` | Runtime | NON-SENSITIVE | COMPLETE: `9e186f4a`. High-value audit-only VRF path wired in `jury_assignment_runtime.py`; epoch-hash shadow preserved as default; `PRODUCTION_ASSIGNMENT_NOT_ACTIVATED=True` unchanged. Exclusion handling for missing/invalid/malformed proof material; ADR-style base64url proof record coverage. 11 new tests; Phase 1396 + 1411 regression 55 passing. Token: `vrf_integration_phase_1412`. `VRF_VERIFIER_IMPLEMENTED` partial met — Phase 1413 tests required for full MET. |
| 1413 | VRF integration tests and security review | Testing | NON-SENSITIVE | COMPLETE: `d75e8267`. 14 tests in `test_phase_1413_vrf_integration.py`; Phase 1411/1412 regression 33 passing. No runtime source changed. Token: `vrf_integration_tests_complete_phase_1413`. Resolves `VRF_VERIFIER_IMPLEMENTED` fully MET (6 of 7 J-008 blocking conditions remain). |
| 1414 | Review lane wiring — T0.5→T1+ admission runtime design and ADR | Spec / ADR | NON-SENSITIVE | COMPLETE: `4622f4be`. ADR-0043 accepted; admission contract and dedup contract defined; Phase 1415–1417 implementation targets specified. Tokens: `review_lane_wiring_adr_accepted_phase_1414`, `t0_5_to_t1_plus_admission_contract_defined_phase_1414`, `review_lane_runtime_not_implemented_phase_1414`. |
| 1415 | Review lane wiring — T0.5→T1+ admission runtime implementation | Runtime | NON-SENSITIVE | COMPLETE: `0d246857`. `ilc_core/epistemic/review_lane_admission_runtime.py` created; all 7 rejection tokens present; `REVIEW_LANE_PRODUCTION_NOT_ACTIVATED=True`; no float/assert/random; `MAX_DEDUP_LOOKUP_RECORDS=10_000` OOM guard; `json.dumps(sort_keys=True, allow_nan=False)`. Tokens: `review_lane_admission_runtime_phase_1415.v0.1`, `review_lane_admission_runtime_implemented_phase_1415`. |
| 1416 | Review lane wiring — dedup enforcement and reviewer-payment settlement stub | Runtime | NON-SENSITIVE | COMPLETE: `0b2a395d`. Dedup (canonical_external_id primary, content-hash fallback), payment stubs with all `_authorized=False` fields, `require_production_review_lane_activation()` fail-closed guard. Tokens: `review_lane_dedup_not_enforced_phase_1415` (archived), reviewer payment `REVIEWER_PAYMENT_NOT_ACTIVATED` preserved. |
| 1417 | Review lane wiring — integration tests; `REVIEW_LANE_WIRING_COMPLETE` token | Testing | NON-SENSITIVE | COMPLETE: `2469820a`. 11 tests covering all rejection tokens, dedup paths, payment stub default-off, AST-level source hygiene, and gate-not-patched verification. Token: `review_lane_wiring_complete_phase_1417`. `REVIEW_LANE_WIRING_COMPLETE` evidence recorded; gate NOT patched (Phase 1425 responsibility). |
| 1418 | Anti-capture diversity — production jury assignment CDL-V3 + VRF wiring design | Spec | NON-SENSITIVE | COMPLETE: `c137a21a`. ADR-0044 accepted; three-check architecture specified: operator-domain independence (already wired), VRF outsider selection (Phase 1412), CDL-V3 cluster floor/ceiling (Phase 1419). Constants: `JURY_CLUSTER_DIVERSITY_FLOOR=4`, `JURY_MAX_CLUSTER_SHARE_CEILING=0.40`. Tokens: `anti_capture_diversity_adr_accepted_phase_1418`, `vrf_outsider_selection_contract_defined_phase_1418`, `anti_capture_diversity_not_verified_phase_1418`. |
| 1419 | Anti-capture diversity — production verification pass; `anti_capture_diversity_verified_phase_1419` token | Runtime/Audit | NON-SENSITIVE | COMPLETE: `5e9d86d3` + Fix1 `33c5ebab`. CDL-V3 cluster diversity wired into `jury_assignment_runtime.py`; `_cluster_diversity_evidence()` raises on floor/ceiling violations; 9 tests (8 base + Fix1 two-cluster regression). `PRODUCTION_ASSIGNMENT_NOT_ACTIVATED=True` unchanged; gate NOT patched. Token: `anti_capture_diversity_verified_phase_1419`. `ANTI_CAPTURE_DIVERSITY_VERIFIED` evidence recorded. |
| 1420 | Copyright counsel disposition — ADR-0041 §5 legal memo or Genesis-authority self-counsel | Governance | SENSITIVE | COMPLETE: `b0b99625`. `docs/specs/ilc_copyright_counsel_disposition_1420_v0.1.md` added; Genesis-authority self-counsel decision; verbatim-storage and D2D-distribution boundaries defined; license-not-public boundary preserved; 9 tests passing. Tokens: `copyright_counsel_disposition_complete_phase_1420`, `self_counsel_verbatim_storage_boundary_phase_1420`, `copyright_counsel_not_external_legal_opinion_phase_1420`, `license_not_public_before_repo_authorized_public_phase_1420`. Gate NOT patched (Phase 1425). |
