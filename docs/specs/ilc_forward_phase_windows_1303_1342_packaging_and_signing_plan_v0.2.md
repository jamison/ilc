...
| 1360 | Multi-operator non-loopback Mysticeti testnet: run 4-validator testnet across geographically distinct VPSs (M-009 was loopback-only); SEC-007a/b dependency updates (tonic 0.13+ upgrade resolving `protoc-bin-vendored` and `rand 0.8.6` Dependabot alerts); **must prove live Python→`ilc_consensus/` gRPC path end-to-end using Phase 1358 adapter against testnet (records `grpc_end_to_end_python_to_rust_proven_phase_1360`)** and confirm `grpcio` as explicit dependency; M-022 proved subprocess/export path only — gRPC path proof is a hard Phase 1366 prerequisite | ADR-0028, M-022 | COMPLETE with Fix1 pending — SEC-007a/b done; tonic 0.14, rand@0.8.6 cleared from lockfile; `grpc_end_to_end_python_to_rust_proven_phase_1360` recorded via insecure-channel testnet proof over Tailscale (production TLS path not exercised; see Fix1 carry-forward); `phase_1360_full_four_validator_epoch_finalization_not_proven` — validator 1 (local control host) crashed before consensus rounds; **Fix1 (`antigravity_prompt__phase_1360_fix1_g8_four_validator_epoch_finalization.md`) must land before Phase 1366 to supply `phase_1360_fix1_four_validator_epoch_finalization_proven`** |
| 1360 Fix1 | Four-validator epoch finalization: diagnose validator 1 crash (expected: macOS QUIC bind failure on Tailscale IP or stale socket); fix config or control script; re-run testnet; collect consensus round log evidence from all four validators; record `phase_1360_fix1_four_validator_epoch_finalization_proven` | ADR-0028, M-022 | NON-SENSITIVE; no CDL mutation; no Rust source change; testnet infrastructure only |
...
**Phase 1360 / 1360 Fix1 carry-forward notes:**
...
- `phase_1360_full_four_validator_epoch_finalization_not_proven` — validator 1 (local control host) crashed before consensus rounds. Phase 1360 Fix1 must land before Phase 1366 to supply `phase_1360_fix1_four_validator_epoch_finalization_proven`. The Phase 1366 gate reviewer should check for this Fix1 token in addition to the Phase 1360 walkthrough tokens.
- Epoch-0 sentinel reconciliation (`get_epoch_chain` from_epoch=0 / to_epoch=0 Rust vs. Python divergence) is still open from Phase 1358 Fix1 — routed to Phase 1360 Fix1 or the phase that activates live ECU transfers, whichever comes first.
