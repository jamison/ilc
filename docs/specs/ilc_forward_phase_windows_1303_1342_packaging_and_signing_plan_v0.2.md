...
- `phase_1360_fix1_epoch_finalization_still_blocked` — Phase 1360 Fix1 corrected the stronger "validator 1 crashed" assumption: validator 1 reaches startup and emits no fatal line, but its local QUIC endpoint remains unreachable after startup; VPS validators still log `stream accept error: closed by peer: 0` before receive/dispatch, and no `epoch_record_committed` evidence was produced. The Phase 1366 gate reviewer should treat four-validator epoch finalization as unresolved unless a later fix records a fresh finalization proof.
