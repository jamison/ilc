...
Where the Phase 454 prose summary and summary-table lead-signal labels are not identical,
Phase 455 resolves the ranking from the row-level output package using the Phase 453
threshold and tie-break rules rather than from shorthand labels alone.
...
| `Scenario 2 - Vesting lock duration discrimination` | `vesting_50_epoch > vesting_25_epoch > vesting_10_epoch > vesting_100_epoch` | `vesting_50_epoch` leads the family on the scenario's primary observables: clamp-respect and release-shock suppression. `vesting_25_epoch` remains the lower-duration/lower-cost challenger, so the family is only partially discriminating rather than fully decisive between the middle pair. |
...
- `Scenario 2 - Vesting lock duration discrimination`: partially discriminating, not cleanly decisive. The family separates the long lock (`100`) and short lock (`10`) from the middle pair, but `vesting_50_epoch` is the lead candidate because it tops the primary observables while `vesting_25_epoch` remains the lower-duration/lower-cost challenger.
