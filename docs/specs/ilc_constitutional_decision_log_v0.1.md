...
| CDL-042 | ADM-003 / CDL-040 / CDL-034 / CDL-001 | Agent identity namespace and self-sovereign ID derivation | open | globally flat namespace with key-derived agent_id, domain-prefixed namespace with operator-scoped agent_id, hierarchical namespace with epoch-scoped key rotation chain | globally flat namespace with key-derived agent_id (proposed) | CDL-001 signing-key anchor, ADM-003 agent-architecture dependency clause, CDL-040 identity-envelope dependency clause, multi-agent-per-operator uniqueness constraint |
