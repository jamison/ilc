...
| --- | --- | --- | --- | --- |
| G1 — Duplicate repo:file nodes | 3,296 `repo:file_ref:*` nodes have no `source_sha256` and no `SAME_SOURCE` edge to their `repo:file:*` counterparts; two representations of the same truth exist in the graph | Fix64 prompt exists: `antigravity_prompt__phase_1545p_fix64_g10_file_ref_content_hash_resolution.md`; resolves via SHA-256 hash + SAME_SOURCE edge write using `AtlasLmdbSafeWriter` | Execute Fix64 before Phase 1573 signing scope finalization; Tier 1 (6 public_protocol_graph nodes) is signing-critical | `implemented_partial` |
| G2 — Content-layer edge types lack recipe annotations | The 13 output edge types of the 6 agent-issuable truth primitives (asserted_by, extends, validated_by, contradicts, refuted_by, supported_by, cites, elaborates, contrasts, instantiates, generalizes, revision_of, revised_by) have no LMDB invariant nodes attesting to their irreducibility; Phase 1387h annotated governance edge types only | No annotation pass exists for content-layer types; `ilc_core/epistemic/truth_primitive_submission_runtime.py` is the authoritative source of graph-output contracts | Fix86 (new annotation pass; `antigravity_prompt__phase_1545p_fix86_g10_content_layer_edge_type_recipe_annotation.md`; NON-SENSITIVE; no CDL) | `implemented_partial` |
