...
The Lane Queue's per-session serial execution with explicit parallel opt-in is exactly the concurrency model our three-AI workflow needs but doesn't formally enforce. Currently, we rely on Genesis operator manually sequencing work across Codex, Antigravity, and Opus. A Lane Queue model would formalize this:
