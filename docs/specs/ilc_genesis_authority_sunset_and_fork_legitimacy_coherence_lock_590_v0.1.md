# ILC Genesis Authority, Sunset, and Fork-Legitimacy Coherence Lock 590 v0.1
...
Phase 590 locks the Genesis authority, sunset, and fork-legitimacy coherence
boundary for Window 585-594.

This packet defines the bounded public consequence of Genesis-rooted lineage,
the ratified recession posture for Genesis authority, and the explicit
carry-forward treatment for the remaining non-equal-canon Genesis questions.
...
Ratified recession anchors `CDL-003`, `CDL-004`, and `CDL-013` are inherited
through the required decision-log row presence in
`docs/specs/ilc_constitutional_decision_log_v0.1.md`; this packet does not
require standalone evidence files for those pre-evidence-era rows.

Tier rule for supporting Genesis context:
- `docs/adr/ADR_0008_Node_Usefulness_vs_Governance_Weight_and_Genesis_Dilution.md`
  is supporting context only, not accepted ADR canon.
- `docs/specs/ilc_freshness_gate_contract_v0.1.md` is supporting context only,
  not ratified public-release law by itself.
- `docs/specs/ilc_genesis_accumulation_dynamics_analysis_298_v0.1.md` is
  analysis only, not protocol law.
- `docs/research/ilc_genesis_authority_and_sunset_canon_briefing_v0.1.md` is a
  tier-labeled research briefing, not binding canon.
- `whitepaper/02_design_principles.md` is rationale only and not self-executing
  protocol law.
- `docs/specs/ilc_pre_epoch_capability_proofs_v0.1.md` remains supporting
  context for a separate post-Genesis capability-proof lane, not current
  public-release law.
- `supporting_genesis_context_not_equal_canon`.
...
assumption or rhetorical promises.

`genesis_dilution_caps_and_emergency_bounds_not_silent_discretion`.

Genesis governance dilution, operational caps, and emergency-path bounds are
constitutional questions that must not be left to silent operator discretion.

`freshness_and_accrual_supporting_context_not_closed_public_law`.

The current freshness and accrual artifacts remain supporting context and not
closed public law by themselves.

The recession posture cites the existing ratified mechanisms at minimum:
- founder fade-out mechanics
- founder operational caps
- globally normalized governance share
- `CDL-V6` sunset and challenge criteria
- `CDL-045` emergency sunset and post hoc review boundary

Unresolved freshness and accrual-governor questions remain explicit
carry-forward canon work, not silent assumptions.
...
or unresolved carry-forward only.

The active carry-forward queue still includes:
- Genesis governance dilution closure
- freshness-gate provenance closure
- Genesis accrual-governor provenance reconciliation

The separate future lane for post-Genesis capability proofs also remains
supporting context until a later ratified vehicle dispositions the CapProof,
AWP/IIH, and optional QATPS/CIT roadmap.
...
Deferred beyond Phase 590:
- public-runtime integration over the frozen boundary to Phase 591
...
- any remaining Genesis dilution, freshness, or accrual-law closure that still requires a dedicated vehicle
