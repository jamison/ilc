...
## 2. Project State (as of Phase 278 completion)
...
- **Current phase:** 278 complete; **Phase 279 next**
...
## 17. SDK/CLI Architecture (NEW in v0.3)

### 17.1 ADM-002 summary

ADM-002 defines a proposed CLI-first Agent SDK interface contract. The command surface maps protocol operations to CLI verbs and keeps orchestration concerns out of the protocol runtime boundary.

Seven primitives as commands:
- `assert`
- `validate`
- `contradict`
- `refute`
...
I/O contract baseline:
- JSON output on stdout,
- structured input via flags/stdin JSON,
- exit codes `0`, `1`, `2`, `3`.

### 17.2 Boundary and routing anchors

- Boundary definition and anti-leakage rules are locked in `docs/specs/ilc_sdk_boundary_contract_234_v0.1.md`.
- CDL routing for CLI-first interface remains `CDL-032` and is currently `open`.
- ADM-002 remains proposed and is not ratified in this capsule update.

## 18. OpenClaw Integration Model (NEW in v0.3)

### 18.1 Role of OpenClaw in ILC architecture

OpenClaw is treated as a distribution and orchestration channel for the CLI-first SDK, not as a hard dependency of protocol correctness.

Integration principles:
- CLI-first interface allows zero custom protocol adapter logic in host orchestration,
- SKILL.md packaging pattern defines install/use semantics for agent frameworks,
- protocol semantics remain independent of host framework lifecycle.

### 18.2 Operational and roadmap placement

- ClawHub publication is a roadmap target in D3 lanes and not a Genesis requirement.
- OpenClaw skill publication contract is routed to `CDL-033` and remains `open`.
- Integration planning references:
...
## 19. Security Runtime Implementation Status (NEW in v0.4)

Security runtime cluster status after Phases 240-244:
- Phase 240: security-runtime sequence lock and lineage lifecycle event schema published.
- Phase 241: `CDL-001` runtime lane implemented for signer-lineage trust-root lifecycle transitions.
- Phase 242: `CDL-002` runtime lane implemented for key-compromise response sequencing.
- Phase 243: `CDL-007` runtime lane implemented for rollback supersession validation and conflict/replay rejection.
- Phase 244: cross-CDL composed integration gate executed and passed.

Open-status reminder:
- `CDL-001`, `CDL-002`, and `CDL-007` remain `open` in the decision log.
- Runtime implementation completion does not imply ratification or status mutation.

Primary anchors:
- `docs/specs/ilc_security_runtime_implementation_sequence_240_249_v0.1.md`
- `docs/specs/ilc_constitutional_decision_log_v0.1.md`

## 20. D2e Pipeline Bootstrap (NEW in v0.4)

D2e documentation-lane status after Phases 245-247:
- Phase 245: ADM-002 v0.2 bootstrap documentation and D2e activation assessment published.
- Phase 246: D2e pipeline scaffolding specification published.
- Phase 247: issuance-governance activation survey for `CDL-025` through `CDL-031` published.

Current D2e boundary:
- D2e remains in documentation and planning lanes.
- D2 schema prerequisites continue to block D2e-03 and later runtime tasks.
- D3/D4/D5 remain explicitly deferred from D2e scaffolding.
...
*End of capsule v0.4. This document should be the first input for every Antigravity phase prompt.*

## 21. CDL Ratification Status (NEW in v0.5)

Ratification state after Phases 251-253:
- `CDL-001`: `ratified` (Phase 251)
- `CDL-002`: `ratified` (Phase 251)
- `CDL-007`: `ratified` (Phase 251)
- `CDL-032`: `ratified` (Phase 253)

Open-state reminder:
- `CDL-019`: `open`
- `CDL-025` through `CDL-031`: `open`
- `CDL-033`: `open`
...
## 22. D2e Specification Progress (NEW in v0.5)

D2e state after Phases 253-257:
- D2e-01 complete: CLI command surface lock (`docs/specs/ilc_cli_command_surface_lock_253_v0.1.md`).
- D2e-02 complete: CLI output schemas (`docs/specs/ilc_cli_output_schemas_254_v0.1.md`).
- D2e-03 readiness assessed: GO for JSON-first prototype with explicit DAG-CBOR deferral (`docs/specs/ilc_d2e_03_readiness_assessment_257_v0.1.md`).

## 23. D2 Schema Foundation (NEW in v0.5)

Schema baseline after Phases 255 and 257:
- Minimal D2 schema specification exists for Node, Edge, and Epoch Record.
- Deterministic vector contracts exist for the same three schemas (golden input, canonical path, CID path, round-trip invariants).
- Remaining D2 schemas stay deferred to subsequent windows.

Primary anchors:
- `docs/specs/ilc_d2_minimal_schema_specification_255_v0.1.md`
- `docs/specs/ilc_d2_schema_test_vectors_spec_257_v0.1.md`

## 24. Issuance Parameter Analysis State (NEW in v0.5)

Issuance-governance state after Phase 256:
- CDL-025 through CDL-031 readiness assessed and ordered for a future ratification lane.
- CDL-019 closure assessed; no ratification executed in this window.
- Issuance values remain unratified and unchanged in decision-log status.

Primary anchor:
- `docs/specs/ilc_issuance_parameter_analysis_256_v0.1.md`

## 25. Wallet-Agnostic Signing Principle (NEW in v0.5)

Wallet-agnostic signing principle adopted.

ILC protocol signing is wallet-agnostic. The signer-lineage registry (CDL-001) tracks lifecycle by opaque identifier; key storage is an operator concern. COSE Sign1 supports secp256k1 natively (RFC 9053). External wallet compatibility (Coinbase, Bitcoin, Ethereum) is architecturally viable. ADM-003 (Reference Agent Architecture) is planned for the 260+ window. A signing-provider interface specification is a pre-D2e-07 planning dependency.

Primary anchor:
- `docs/specs/ilc_wallet_agnostic_signing_strategy_codex_handoff_v0.1.md`

## 26. Issuance Governance Closure State (NEW in v0.6)

Issuance-governance closure status for window 270-277:
- `CDL-029`: `ratified` (Phase 272)
- `CDL-026`: `ratified` (Phase 273)
- `CDL-028`: `ratified` (Phase 274)
- `CDL-027`: `ratified` (Phase 276)
- `CDL-030`: `ratified` (Phase 277)

Deferred/open reminder:
- `CDL-031` remains `open` and unratified.

Closure interpretation:
- The issuance model baseline (Model B + governed constant + invariant floor) now has all non-`CDL-031` parameters ratified in this window.
...
## 27. CDL-031 Status (NEW in v0.6)

`CDL-031` remains open by design in this window.

Status statement:
- `CDL-031` was unblocked after `CDL-019` closure in Phase 268.
- It remains explicitly deferred pending a dedicated ranking-policy evidence lane in Phase 280+.
- No `CDL-031` mutation or ratification occurred in Phases 270-278.
...
*End of capsule v0.6. This document should be the first input for every Antigravity phase prompt.*
