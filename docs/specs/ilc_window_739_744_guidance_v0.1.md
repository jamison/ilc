...
- `SIM-LEAKAGE-01` commissioning spec:
  expected source is Gemini `M-019` multi-machine traffic evidence
- row-7 runtime evidence package:
  live censorship-resistance and exitability confirmation requirements tied to
  the already-closed spec and TLC evidence
...
- Do not claim row 5 runtime closure unless live operator-path leakage evidence
  actually exists
- Do not claim row 7 runtime closure unless live censorship-and-exitability
  runtime confirmation actually exists
...
| `SIM-LEAKAGE-01` commissioning spec | `docs/specs/ilc_sim_leakage_01_commissioning_spec_740_v0.1.md` | Expected evidence source: Gemini `M-019` multi-machine traffic capture |
| Row-7 runtime evidence package | `docs/specs/ilc_row_7_runtime_evidence_package_741_v0.1.md` | Must tie Phase 674 threshold + Phase 698 TLC proof to live runtime confirmation requirements |
...
| `CDL-068` ratification artifact | `docs/specs/ilc_cdl_068_topology_shuffle_authorization_ratification_evidence_743_v0.1.md` | Must explicitly check all six Phase 736 prelock criteria |
...
| 740 | Row-5 runtime evidence package — formalize what the live operator-path leakage measurement must contain; publish `SIM-LEAKAGE-01` commissioning spec; update row-5 status documentation |
| 741 | Row-7 runtime evidence package — formalize exitability and censorship-resistance runtime confirmation requirements; connect Phase 698 TLC proof to expected Gemini `M-019` output; publish updated row-7 status artifact |
...
| 743 | `CDL-068` ratification — write and publish ratification text, evidence-checklist satisfaction record, and decision-log mutation |
...
1. What exact live evidence would be sufficient to move row 5 from
   ...
   evidence must come from Gemini `M-019` rather than Codex docs work?
...
4. What count as the minimum runtime scenarios for row 7:
   censoring validator, retry path, export / replay / migrate drill, or a
   stronger bundle?
5. Is `CDL-068` ready for ratification independently of rows 5 and 7?
...
7. `docs/specs/ilc_dag_censorship_bounds_tlc_evidence_698_v0.1.md`
8. `docs/specs/ilc_exitability_and_replayability_threshold_674_v0.1.md`
9. `docs/specs/ilc_cdl_068_topology_shuffle_authorization_opening_v0.1.md`
10. `docs/research/ilc_sim_validator_01_results_v0.1.md`
11. `docs/research/ilc_sim_topology_01_results_v0.1.md`
12. `docs/research/ilc_validator_agent_design_evidence_v0.1.md`
13. `docs/specs/ilc_cdl_039_topology_shuffling_authorization_scope_note_711_v0.1.md`
14. `docs/specs/ilc_m_series_test_coverage_and_hardening_plan_v0.1.md`
15. `docs/research/ilc_mysticeti_implementation_lane_m_series_v0.1.md`

---

## 9. No pre-window conversation required

Window `739-744` has no pre-window conversation gate. This guidance doc is
sufficient for packet drafting and standard review.
