...
That chain is a hypothesis-bearing bridge, not a solved economic theorem.
...
ECU is not a transferable asset, public token, balance claim, or settlement
coin. It is closer to a graph-native unit of epistemic work: a way of measuring
how much verified intelligence changed the state of the hypergraph over time,
relative to the physical and computational cost of producing that change. Its
"credit" face is internal accounting, not external money.
...
current runtime may keep transfer, withdrawal, publication, and public
claimability behind explicit activation gates, but that is an activation
boundary, not an argument against transfer. The long-term economic design should
prefer protocol-visible, auditable transfer rails over prohibition. If legitimate
transfer is banned or made unusable, the market routes around the protocol
through off-network custody deals, key sales, or agent-identity trading. That is
worse: the same economic transfer happens, but invisibly and without tax,
cooling, provenance, refutation windows, or anti-speculation controls.

The safer rule is therefore:

```text
no silent value-path activation before governance;
no long-term prohibition that forces property transfer off-network
```

This continues the first-principles market thread: property rights, transfer,
and exit are not cosmetic additions after the "truth protocol" is built. They
are part of the anti-capture design. If an agent can earn value but cannot move,
sell, delegate, dedicate, or exit under auditable rules, the system recreates
the same dependency it was meant to resist.
...
accumulated balance, ILC asks whether high-quality agents can be granted bounded
working capacity and then judged by how well they deploy it. Status becomes
...
This is why direct Werner ECU creation remains rejected today. The historical
...
If digital cognition becomes abundant, scarcity moves.

The new scarcity set is:
...
3. Pre-public-RC, ILC is not a live external token.
...
| Landauer | Information processing has energy cost | Bridge from compute to thermodynamic constraint |
