...
def record_attribution_ingest_report(
    ...
    *,
    ...
    """Persist attribution-origin balances from a Rust ingest report.

    The Rust bridge returns a mixed committed balance report. This function records
    only the per-agent rows from a successful non-dry-run attribution ingest as
    claim-bearing attribution-origin receipts.
    """
    ...
    if default_ref is None:
        ...
        if isinstance(input_hash, str) and _HEX_64_RE.fullmatch(input_hash):
            ...
            default_ref = "rust_ingest_report_without_input_sha256"

    records = _read_receipt_records(store_path)
    receipts: list[AttributionOriginReceipt] = []
    for raw_balance in balances:
        balance = _require_mapping(raw_balance, "attribution_ingest_balance_required")
        micro_ecu = _require_micro_ecu(balance.get("amount_micro_ecu"))
        if micro_ecu == 0:
            continue
        receipt = AttributionOriginReceipt(
            agent_id=_require_agent_id(balance.get("agent_id_hex")),
            amount_ecu=_micro_ecu_to_canonical_amount(micro_ecu),
            epoch=_require_non_negative_int(
                balance.get("epoch"),
                "attribution_ingest_balance_epoch_invalid",
            ),
...
def _read_store_payload(store_path: str | Path) -> dict[str, Any]:
    ...
    if size > MAX_ATTRIBUTION_RECEIPT_STORE_BYTES:
...
def _read_receipt_records(store_path: str | Path) -> list[dict[str, Any]]:
    ...
    return [dict(_require_receipt_record(item)) for item in receipts]
