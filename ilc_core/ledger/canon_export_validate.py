...
def _validate_required_keys(doc: Dict[str, Any], errors: List[str]) -> bool:
...
def _validate_epochs(epochs: Any, errors: List[str]) -> None:
...
def _validate_snapshot_balances(idx: int, balances: Any, errors: List[str]) -> None:
...
def _validate_snapshots(snapshots: Any, errors: List[str]) -> None:
...
def _add_warnings(doc: Dict[str, Any], warnings: List[str], epochs: Any, snapshots: Any) -> None:
    ...
    if isinstance(epochs, list) and isinstance(snapshots, list) and len(epochs) == 0 and len(snapshots) == 0:
