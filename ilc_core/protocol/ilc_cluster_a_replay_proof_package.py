...
def verify_cluster_a_replay_proof_package(package: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if claimed_pkg_hash != computed_pkg_hash:
        ...
        return {"ok": False, "errors": errors, "checks": checks} # Fail fast on hash mismatch
    ...
    # Also check evidence's record hash match?
    # Evidence validation in attest will check evidence["record_hash_sha256"] vs gov_rec hash.
    # But we should check package top-level vs evidence top-level?
    if evidence.get("record_hash_sha256") != claimed_rec_hash:
        # This technically falls under "evidence record hash mismatch" if we rely on attest.
        # But let's check explicit package integrity.
        pass # Let attest handle it.
