...
Not every graph node is an observer. Most nodes are static artifacts — a measurement, a transcript, a theorem, a signed refutation. Agents are the active class: identities capable of acting, querying, and emitting new deltas. The observer emits; the graph remembers. The boundary between the living observer and the committed record is the fundamental asymmetry on which ILC's economic architecture rests.
