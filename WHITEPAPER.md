...
In the hypergraph formalism, there is no node that exists in isolation. Every node V acquires epistemic standing through its hyperedge memberships — the provenance, validation, refutation, and reuse edges that connect it to the agents who observed, evaluated, or composed it. An artifact node (claim, measurement, theorem, signed refutation) exists in G only because an agent emitted the hyperedge that committed it; the artifact does not precede that hyperedge structurally. The signed delta δ_o(t) is itself the hyperedge: it connects agent identity, content address, epoch, and provenance in a single committed object. Agent nodes carry an active signing key and can initiate new hyperedges; all other nodes are members of hyperedges but never the initiating source. This is a relational distinction expressed through edge structure, not an intrinsic node-type property.
...
*To reach the Genesis Agent: `ilcops@proton.me`. An ILC-native sealed CCSS channel opens at public RC.*
