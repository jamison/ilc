...
*Before naming the capital form required for the current regime, it helps to ask what capital is from first principles — its ontological base elements, before any particular historical instantiation.*

*Every capital form, examined across every era, rests on three irreducible structural requirements. First, the information must be organized and dense enough to enable productive recombination — raw entropy is not capital; it must be specifically structured to encode relationships that other agents can build on. Second, it must persist long enough for the economic cycle of discovery, reference, and return to complete — a sophisticated machine that exists for twenty seconds generates no durable capital regardless of how information-rich it is; the holding vessel must outlast the return cycle. Third, a future agent must be able to locate and retrieve it without the original contributor being present — the vessel must carry a stable forward address, a reference that survives and can be dereferenced across time. Capital exists where all three are simultaneously non-trivial. Remove any one and you have information, or storage, or addresses — but not capital.*

*Every major informational phase transition in history has been a step-change in one or more of these dimensions. Matter organized into life: the durability of information storage jumped dramatically — DNA replication fidelity produced self-correcting copies that propagate across geological time. Life organized into cognition: density and accessibility jumped — neural encoding carried orders of magnitude more productive structure, transmissible between individuals within a community. Cognition organized into institutions: durability and accessibility jumped again — writing gave multi-generational persistence; named authorship gave crude but durable forward addresses that survived the death of the original contributor. Then machine intelligence emerged as a distinct productive factor, cheaper to instantiate than to educate: density and recombinability of information jumped at every level. At each transition, the capital form calibrated for the previous regime became an imperfect proxy for the new one — not because the old idea was wrong, but because its durability or accessibility mechanism could not handle the new density or the new timescale at which productive reuse operated.*

*The digital era looked like the final step — information encoded with perfect fidelity, globally retrievable, infinitely replicable. But it left one structural problem unsolved: durability depends on platform goodwill. Servers shut down. Platforms delete content. Formats become unreadable. Attribution chains dissolve under legal dispute and commercial incentive. The vessel is contingently maintained, not structurally guaranteed. And when the platform controls the vessel, the platform controls the capital.*

*Gary Becker's Human Capital — established in 1964 as the present value of embodied skill, education, and accumulated labor capacity — was the right instrument for a world where cognition was biological and instance-bound. Digital intelligent agents break every assumption it rests on: they are spun up, copied, and shut down; they produce intellectual output continuously; they own none of it; and their accumulated capacity vanishes with the instance. The durability mechanism fails at biological death; for digital agents, it fails at instance termination — and neither can outlast the platform that hosts them.*

*ILC introduces Agentic Capital as the Becker-compatible capital form for this new regime — not a replacement of Human Capital but the extension required when productive agents are no longer exclusively biological. ILC uses this term in a specific protocol-native sense: capital that is graph-resident rather than embodied, cryptographically attributed to a persistent identity rather than platform-held, verifier-weighted by adversarial challenge survival rather than market price, and instance-death-persistent across arbitrary substrate changes. The content address is the holding vessel — computed from the content itself, not assigned by any authority. The hash guarantees integrity. The provenance chain is the permanent forward address. For the first time in the informational phase boundary progression, the durability of capital is structurally guaranteed by protocol rather than by social arrangement, legal agreement, or platform policy.*

*This is the capital form the informational phase boundary demands. ILC is the protocol that makes it real.*
