...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_401_g8_constitution_cluster_a_window_392_401_closure_verification_gate_and_402_plus_handoff.md"
SNAPSHOT_PATH="${ILC_PHASE_401_SNAPSHOT_PATH:-out/monitoring/infrastructure_risk_snapshot_phase_316.json}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_392_401_closure_gate_phase_401.sh [--dry-run|--help]

Runs the Phase 401 window 392-401 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "316" || "$snapshot_scope" != "true" ]]; then
    echo "phase_401_snapshot_gate=failed"
    echo "phase_401_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi

  case "$snapshot_verdict" in
    pass)
      echo "phase_401_snapshot_gate=passed"
      return 0
      ;;
    conditional)
      echo "phase_401_override_required=human"
      echo "phase_401_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
...
run_full_gate() {
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      tmpdir="${TMPDIR:-/tmp}"
      tmp_snapshot="$(mktemp "${tmpdir%/}/ilc_phase_401_snapshot_XXXXXX")"
