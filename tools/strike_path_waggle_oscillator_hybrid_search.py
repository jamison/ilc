...
OUTPUT_DIR = Path('out/strike_path/waggle_oscillator_hybrid_search_2026-03-27')
...
def estimate_oscillator(period: int, amplitude: float, phase_offset: str) -> dict[str, float | int]:
    period_term = _gauss(float(period), 5.0, 1.2)
    amplitude_term = _gauss(amplitude, 0.09, 0.022)
    release_first = phase_offset == 'release_first'
    long_period_penalty = 0.035 * max(0.0, float(period - 6)) / 2.0

    organic = 0.83 + 0.010 + 0.067 * period_term + 0.043 * amplitude_term
    if release_first:
        organic -= 0.012
    organic -= long_period_penalty

    clamp = 0.82 + 0.008 + 0.036 * period_term + 0.026 * amplitude_term
    if release_first:
        clamp -= 0.004

    duration = int(round(8.0 + 0.9 * (period - 4) + 14.0 * abs(amplitude - 0.09) + (0.6 if release_first else 0.0)))
    cost = 0.20 + 0.008 * period + 0.18 * amplitude + (0.008 if release_first else 0.0)
...
def estimate_hybrid(
    ...
    oscillator_metrics = estimate_oscillator(oscillation_period, oscillation_amplitude, phase_offset)

    knee_term = _gauss(float(waggle_parameters['response_knee']), 0.58, 0.07)
    gain_term = _gauss(float(waggle_parameters['control_gain']), 1.85, 0.35)
    floor_term = _gauss(float(waggle_parameters['release_floor']), 0.22, 0.05)
    ...
    if bias == 'frontier_first':
        selectivity_term = 1.0
    elif bias == 'uniform':
        selectivity_term = 0.78
    else:
        selectivity_term = 0.64

    phase_term = 1.0 if phase_offset == 'enforce_first' else 0.58
    gate_quality = 0.55 * period_term + 0.45 * amplitude_term
    waggle_quality = 0.40 * knee_term + 0.35 * gain_term + 0.25 * floor_term

    organic_synergy = 0.0065 * gate_quality * waggle_quality * phase_term * selectivity_term
    organic_synergy += 0.0025 * amplitude_term * floor_term * phase_term

    clamp_synergy = 0.0045 * gate_quality * (0.65 + 0.35 * gain_term) * phase_term * selectivity_term
    clamp_synergy += 0.0015 * period_term * gain_term

    organic = max(
        float(waggle_metrics['organic_ecu_production_rate']),
        float(oscillator_metrics['organic_ecu_production_rate']),
    ) + organic_synergy
    clamp = max(
        float(waggle_metrics['pe_clamp_respect_rate']),
        float(oscillator_metrics['pe_clamp_respect_rate']),
    ) + clamp_synergy

    duration = max(
        int(waggle_metrics['intervention_duration_epochs']),
        int(oscillator_metrics['intervention_duration_epochs']),
    ) + 1
    if phase_offset == 'release_first':
        duration += 1

    cost = max(
        float(waggle_metrics['intervention_cost_units']),
        float(oscillator_metrics['intervention_cost_units']),
    ) + 0.010 + 0.004 * (1.0 - selectivity_term)
...
def _score_candidate(
    ...
    score += org_gap_tuned * 20.0 + clamp_gap_tuned * 16.0
    score += org_gap_waggle * 16.0 + clamp_gap_waggle * 12.0
...
def _evaluate_state(
    ...
    waggle_source_module, metrics = estimate_hybrid(
...
def _mutate_state(state: HybridState, ranked_waggle_ids: list[str]) -> list[HybridState]:
    ...
    mutations.append(
        HybridState(
            ...
            oscillation_period=state.oscillation_period,
            oscillation_amplitude=state.oscillation_amplitude,
            phase_offset='release_first' if state.phase_offset == 'enforce_first' else 'enforce_first',
            generation=state.generation + 1,
        )
    )
    ...
    for delta in (-5, -2, -1, 1, 2, 5):
        ...
        if 0 <= next_index < len(ranked_waggle_ids):
            mutations.append(
                HybridState(
                    waggle_candidate_id=ranked_waggle_ids[next_index],
                    oscillation_period=state.oscillation_period,
                    oscillation_amplitude=state.oscillation_amplitude,
                    phase_offset=state.phase_offset,
                    generation=state.generation + 1,
    ...
    for generation in range(generations):
        ...
        for candidate in candidates:
            candidate = HybridState(
                ...
                oscillation_period=candidate.oscillation_period,
                oscillation_amplitude=candidate.oscillation_amplitude,
        ...
        generation_leader = estimates[0]
        generation_summaries.append(
            {
                'generation': generation,
                'evaluated_candidates': len(estimates),
                'leader': asdict(generation_leader),
            }
        )
...
def _write_outputs(estimates: Iterable[HybridEstimate], generation_summaries: list[dict[str, object]]) -> None:
    ...
    payload = {
        'method': 'Strike Path: Waggle-Oscillator Hybrid Autoresearch',
    ...
    lines = [
        '# Strike Path Waggle-Oscillator Hybrid Search',
        '',
        'Method: fixed-evaluator autoresearch-style beam search over hybrid candidates.',
        '',
        '| Rank | Gen | Waggle candidate | Oscillator | Org | Clamp | Org gap vs anchor | Clamp gap vs anchor | Org delta vs tuned osc | Clamp delta vs tuned osc | Slack | Duration | Cost |',
        '| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |',
    ]
    for rank, item in enumerate(estimates[:12], start=1):
        ...
        lines.append(
            ...
            f'{item.organic_gap_vs_anchor:.3f} | {item.clamp_gap_vs_anchor:.3f} | '
...
def main() -> int:
    ...
    leader = estimates[0]
    print('Strike Path: Waggle-Oscillator Hybrid Autoresearch')
    ...
    print(
        'Projected deltas: '
        f'vs anchor org={leader.organic_gap_vs_anchor:.3f}, clamp={leader.clamp_gap_vs_anchor:.3f}; '
        f'vs tuned oscillator org={leader.organic_gap_vs_tuned_oscillator:.3f}, '
        f'clamp={leader.clamp_gap_vs_tuned_oscillator:.3f}; '
        f'slack={leader.threshold_slack:.3f}'
    )
    return 0
