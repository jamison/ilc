...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_504_g8_window_495_504_closure_gate_and_handoff.md"
DECISION_LOG_PATH="${ILC_PHASE_504_DECISION_LOG_PATH:-docs/specs/ilc_constitutional_decision_log_v0.1.md}"
SNAPSHOT_PATH="${ILC_PHASE_504_SNAPSHOT_PATH:-}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_495_504_closure_gate_phase_504.sh [--dry-run|--help]

Runs the Phase 504 window 495-504 closure gate.
...
elif rows.get('CDL-055', {}).get('status') == 'ratified' and rows.get('CDL-056', {}).get('status') == 'ratified':
    print('success|cdl_055_and_cdl_056_ratified')
elif rows.get('CDL-055', {}).get('status') == 'ratified' and 'CDL-056' not in rows:
    print('blocked|cdl_056_not_opened')
else:
...
run_command() {
  ...
  env \
    ...
    -u ILC_PHASE_504_DECISION_LOG_PATH \
    -u ILC_PHASE_504_SNAPSHOT_PATH \
    "$@" bash -c "$command"
}
...
run_full_gate() {
  ...
  case "$state" in
    success)
      echo "phase_504_window_state=success_path"
      ;;
    blocked)
      echo "phase_504_window_state=blocked_path"
      ;;
    invalid)
      echo "phase_504_window_state=invalid"
      echo "phase_504_window_state_details=${detail}"
      write_snapshot "$state" "$detail"
      return 1
      ;;
    *)
      echo "phase_504_window_state=invalid"
      echo "phase_504_window_state_details=unrecognized_state"
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      run_command "${commands[$idx]}" ILC_PHASE_504_GATE_SELFTEST=1
    else
      run_command "${commands[$idx]}"
    fi
  done
  write_snapshot "$state" "$detail"
  echo "phase_504_verdict=pass"
}
