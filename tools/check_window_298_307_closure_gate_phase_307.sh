...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_298_307_closure_gate_phase_307.sh [--dry-run|--help]

Runs the Phase 307 window 298-307 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "306" || "$snapshot_scope" != "true" ]]; then
    echo "phase_307_snapshot_gate=failed"
    echo "phase_307_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi
  ...
  case "$snapshot_verdict" in
    pass)
      ...
      ;;
    conditional)
      echo "phase_307_override_required=human"
      echo "phase_307_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
