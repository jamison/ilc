...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_524_g8_window_515_524_closure_gate_and_handoff.md"
DECISION_LOG_PATH="${ILC_PHASE_524_DECISION_LOG_PATH:-docs/specs/ilc_constitutional_decision_log_v0.1.md}"
EPOCH_RUNTIME_PATH="${ILC_PHASE_524_EPOCH_RUNTIME_PATH:-ilc_core/epoch/epoch_boundary_witness_runtime.py}"
READMISSION_RUNTIME_PATH="${ILC_PHASE_524_READMISSION_RUNTIME_PATH:-ilc_core/validator/re_admission_runtime.py}"
SNAPSHOT_PATH="${ILC_PHASE_524_SNAPSHOT_PATH:-}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_515_524_closure_gate_phase_524.sh [--dry-run|--help]

Runs the Phase 524 window 515-524 closure gate.
...
elif not epoch_runtime.exists():
    print('invalid|cdl_057_runtime_missing')
else:
    cdl_058_status = rows.get('CDL-058', {}).get('status')
    readmission_exists = readmission_runtime.exists()
    if cdl_058_status == 'ratified' and readmission_exists:
        print('success|cdl_058_ratified_and_runtime_present')
    elif cdl_058_status in {None, 'open'} and not readmission_exists:
        print('blocked|cdl_058_not_yet_ratified')
    elif cdl_058_status == 'ratified' and not readmission_exists:
        print('invalid|cdl_058_ratified_but_runtime_missing')
    elif cdl_058_status in {None, 'open'} and readmission_exists:
        print('invalid|cdl_058_not_ratified_but_runtime_present')
    else:
        print('invalid|unexpected_re_admission_state')
PY
}
...
run_command() {
  ...
  env \
    ...
    -u ILC_PHASE_524_DECISION_LOG_PATH \
    -u ILC_PHASE_524_EPOCH_RUNTIME_PATH \
    -u ILC_PHASE_524_READMISSION_RUNTIME_PATH \
    -u ILC_PHASE_524_SNAPSHOT_PATH \
    "$@" bash -c "$command"
}
...
run_full_gate() {
  ...
  case "$state" in
    success)
      echo "phase_524_window_state=success_path"
      ;;
    blocked)
      echo "phase_524_window_state=blocked_path"
      ;;
    invalid)
      echo "phase_524_window_state=invalid"
      echo "phase_524_window_state_details=${detail}"
      write_snapshot "$state" "$detail"
      return 1
      ;;
    *)
      echo "phase_524_window_state=invalid"
      echo "phase_524_window_state_details=unrecognized_state"
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      run_command "${commands[$idx]}" ILC_PHASE_524_GATE_SELFTEST=1
    else
      run_command "${commands[$idx]}"
    fi
  done
  write_snapshot "$state" "$detail"
  echo "phase_524_verdict=pass"
}
