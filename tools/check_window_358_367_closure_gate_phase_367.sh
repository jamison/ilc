...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_367_g8_constitution_cluster_a_window_358_367_closure_verification_gate_and_368_plus_handoff.md"
SNAPSHOT_PATH="${ILC_PHASE_367_SNAPSHOT_PATH:-out/monitoring/infrastructure_risk_snapshot_phase_316.json}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_358_367_closure_gate_phase_367.sh [--dry-run|--help]

Runs the Phase 367 window 358-367 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "316" || "$snapshot_scope" != "true" ]]; then
    echo "phase_367_snapshot_gate=failed"
    echo "phase_367_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi

  case "$snapshot_verdict" in
    pass)
      echo "phase_367_snapshot_gate=passed"
      return 0
      ;;
    conditional)
      echo "phase_367_override_required=human"
      echo "phase_367_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
...
run_full_gate() {
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      ILC_PHASE_367_GATE_SELFTEST=1 bash -c "${commands[$idx]}"
    else
      bash -c "${commands[$idx]}"
    fi
  done

  echo "phase_367_verdict=pass"
  return 0
}
