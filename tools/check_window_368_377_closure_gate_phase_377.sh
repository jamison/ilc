...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_377_g8_constitution_cluster_a_window_368_377_closure_verification_gate_and_378_plus_handoff.md"
SNAPSHOT_PATH="${ILC_PHASE_377_SNAPSHOT_PATH:-out/monitoring/infrastructure_risk_snapshot_phase_316.json}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_368_377_closure_gate_phase_377.sh [--dry-run|--help]

Runs the Phase 377 window 368-377 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "316" || "$snapshot_scope" != "true" ]]; then
    echo "phase_377_snapshot_gate=failed"
    echo "phase_377_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi

  case "$snapshot_verdict" in
    pass)
      echo "phase_377_snapshot_gate=passed"
      return 0
      ;;
    conditional)
      echo "phase_377_override_required=human"
      echo "phase_377_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
...
run_full_gate() {
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      tmpdir="${TMPDIR:-/tmp}"
      tmp_snapshot="$(mktemp "${tmpdir%/}/ilc_phase_377_snapshot_XXXXXX")"
      ...
      ILC_PHASE_377_GATE_SELFTEST=1 bash -c "${commands[$idx]}"
    else
      bash -c "${commands[$idx]}"
    fi
  done

  echo "phase_377_verdict=pass"
  return 0
}
