...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_433_g8_window_424_433_closure_gate_and_434_plus_handoff.md"
SNAPSHOT_PATH="${ILC_PHASE_433_SNAPSHOT_PATH:-out/monitoring/infrastructure_risk_snapshot_phase_316.json}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_424_433_closure_gate_phase_433.sh [--dry-run|--help]

Runs the Phase 433 window 424-433 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "316" || "$snapshot_scope" != "true" ]]; then
    echo "phase_433_snapshot_gate=failed"
    echo "phase_433_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi

  case "$snapshot_verdict" in
    pass)
      echo "phase_433_snapshot_gate=passed"
      return 0
      ;;
    conditional)
      echo "phase_433_override_required=human"
      echo "phase_433_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
...
run_full_gate() {
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      tmpdir="${TMPDIR:-/tmp}"
      tmp_snapshot="$(mktemp "${tmpdir%/}/ilc_phase_433_snapshot_XXXXXX")"
      if env \
        -u ILC_PHASE_433_SNAPSHOT_PATH \
        -u ILC_PHASE_423_SNAPSHOT_PATH \
        -u ILC_PHASE_413_SNAPSHOT_PATH \
