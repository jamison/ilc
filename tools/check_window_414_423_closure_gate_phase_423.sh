...
PROMPT_PATH="docs/antigravity_tasks/antigravity_prompt__phase_423_g8_window_414_423_closure_gate_and_424_plus_handoff.md"
SNAPSHOT_PATH="${ILC_PHASE_423_SNAPSHOT_PATH:-out/monitoring/infrastructure_risk_snapshot_phase_316.json}"
...
usage() {
  cat <<'USAGE'
Usage: tools/check_window_414_423_closure_gate_phase_423.sh [--dry-run|--help]

Runs the Phase 423 window 414-423 closure verification gate.
...
resolve_gate_verdict() {
  ...
  if [[ "$snapshot_phase" != "316" || "$snapshot_scope" != "true" ]]; then
    echo "phase_423_snapshot_gate=failed"
    echo "phase_423_snapshot_details=phase:${snapshot_phase},preflight_scope:${snapshot_scope}"
    return 1
  fi

  case "$snapshot_verdict" in
    pass)
      echo "phase_423_snapshot_gate=passed"
      return 0
      ;;
    conditional)
      echo "phase_423_override_required=human"
      echo "phase_423_snapshot_gate=failed"
      return 3
      ;;
    blocked|*)
...
run_full_gate() {
  ...
  for idx in "${!labels[@]}"; do
    ...
    if [[ "$idx" -eq 2 ]]; then
      ...
      tmpdir="${TMPDIR:-/tmp}"
      tmp_snapshot="$(mktemp "${tmpdir%/}/ilc_phase_423_snapshot_XXXXXX")"
      if env \
        -u ILC_PHASE_423_SNAPSHOT_PATH \
        -u ILC_PHASE_413_SNAPSHOT_PATH \
        -u ILC_PHASE_401_SNAPSHOT_PATH \
