"""Phase 367 closure gate contract tests for window 358-367."""
...
GATE_PATH = Path("tools/check_window_358_367_closure_gate_phase_367.sh")
GATE_CMD = ["bash", str(GATE_PATH)]
HANDOFF_PATH = Path("docs/specs/ilc_window_358_367_handoff_367_v0.1.md")
SNAPSHOT_PATH = Path("out/monitoring/infrastructure_risk_snapshot_phase_316.json")
DECISION_LOG_PATH = "docs/specs/ilc_constitutional_decision_log_v0.1.md"
PHASE_367_COMMIT_SUBJECT = "docs(g8): phase 367 window 358-367 closure gate and 368-plus handoff"
...
def _snapshot_override_env(tmp_path: Path, verdict: str) -> tuple[dict[str, str], bytes, int, str]:
    ...
    env["ILC_PHASE_367_SNAPSHOT_PATH"] = str(snapshot_copy)
...
def _resolve_phase_367_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if subject.strip() == PHASE_367_COMMIT_SUBJECT:
    ...
    if matching_commits:
        raise AssertionError("phase_367_commit_subject_present_but_no_qualifying_closure_commit")
    raise AssertionError("phase_367_commit_not_present_in_local_history")
...
def test_gate_cli_contract_dry_run_help_alias_unknown_arg_and_exact_lines() -> None:
    ...
    assert lines == [
        ...
        "python3 tools/validate_phase_prompt.py docs/antigravity_tasks/antigravity_prompt__phase_367_g8_constitution_cluster_a_window_358_367_closure_verification_gate_and_368_plus_handoff.md",
        "[2/6] lane_contract_tests",
        "python3 -m pytest tests/test_phase_358_sequence_lock.py tests/test_cdl_039_open_and_p2p_transport_baseline_prelock_359.py tests/test_node_schema_core_runtime_360.py tests/test_validation_lifecycle_runtime_361.py tests/test_node_dissemination_runtime_362.py tests/test_executable_descriptor_runtime_363.py tests/test_promotion_continuity_runtime_364.py tests/test_phase_365_sim_commissioning.py tests/test_phase_366_coherence_and_capsule_v1_1.py -q",
        ...
        "python3 -m pytest tests/test_window_358_367_closure_gate_367.py -q",
...
def test_gate_full_run_exits_zero_on_pass_snapshot_and_preserves_canonical_snapshot() -> None:
    if os.environ.get("ILC_PHASE_367_GATE_SELFTEST") == "1":
        pytest.skip("phase_367_selftest_context_skip_full_gate")
    ...
    assert "phase_367_snapshot_gate=passed" in result.stdout
    assert "phase_367_verdict=pass" in result.stdout
...
def test_gate_full_run_enforces_conditional_exit_and_snapshot_isolation(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_367_GATE_SELFTEST") == "1":
        pytest.skip("phase_367_selftest_context_skip_verdict_simulation")
    env, canonical, canonical_mtime, canonical_sha = _snapshot_override_env(tmp_path, "conditional")
    result = _run_gate([], env=env)
    assert result.returncode == 3
    assert "phase_367_override_required=human" in result.stdout
    assert "phase_367_snapshot_gate=failed" in result.stdout
...
def test_no_decision_log_mutation_in_phase_commit() -> None:
    if os.environ.get("ILC_PHASE_367_GATE_SELFTEST") == "1":
        pytest.skip("phase_367_selftest_context_skip_commit_guardrails")
    commit_ref = _resolve_phase_367_commit_ref()
...
def test_no_ilc_core_runtime_feature_mutation_in_phase_commit() -> None:
    ...
    assert not forbidden, f"phase_367_runtime_feature_mutations:{forbidden}"
