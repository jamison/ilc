...
def test_ci_workflow_runs_main_track_return_preflight_214_219_gate() -> None:
    workflow = WORKFLOW_PATH.read_text(encoding="utf-8")
    assert "bash tools/check_main_track_return_preflight_214_219.sh" in workflow
...
def test_ci_workflow_main_track_preflight_runs_before_full_pytest() -> None:
    ...
    idx_preflight_214_219 = workflow.index("bash tools/check_main_track_return_preflight_214_219.sh")
    idx_full_pytest = workflow.index("python -m pytest -q")
    assert idx_preflight_193_199 < idx_preflight_203_209 < idx_preflight_214_219 < idx_full_pytest
