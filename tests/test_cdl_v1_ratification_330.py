...
EVIDENCE_PATH = Path("docs/specs/ilc_cdl_v1_temporal_decay_ratification_evidence_330_v0.1.md")
DECISION_LOG_PATH = Path("docs/specs/ilc_constitutional_decision_log_v0.1.md")
PHASE_324_TEST_PATH = Path("tests/test_cdl_v_batch_a_open_and_evidence_prelock_324.py")
PHASE_330_COMMIT_SUBJECT = "docs(g8): phase 330 cdl-v1 ratification"
...
def _resolve_phase_330_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if subject.strip() == PHASE_330_COMMIT_SUBJECT:
    ...
    if matching_commits:
        raise AssertionError("phase_330_commit_subject_present_but_no_qualifying_ratification_commit")
    raise AssertionError("phase_330_commit_not_present_in_local_history")
...
def test_ratification_evidence_file_exists_and_has_required_content() -> None:
    ...
    for token in (
        ...
        "Section 3 of `docs/specs/ilc_cdl_v1_temporal_decay_evidence_prelock_324_v0.1.md` is authoritative",
        "`decay sensitivity analysis`",
        "`lock-in displacement simulation`",
        "`monitoring threshold proposal`",
        "`parameter-boundary discussion for governance control and rollback safety`",
...
def test_phase_324_historical_prelock_hardening_patch_is_active() -> None:
    ...
    assert 'for cdl_id in ("CDL-V2", "CDL-V3")' in text
    assert "historical prelock reference" in text


def test_decision_log_cdl_v1_is_ratified_with_expected_metadata() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    row = rows["CDL-V1"]
    assert row["status"] == "ratified"
    assert row["ratified_phase"] == "330"
    ...
    assert (
        ...
        == "docs/specs/ilc_cdl_v1_temporal_decay_ratification_evidence_330_v0.1.md"
    )


def test_mutation_scope_guardrail_allows_only_ratification_fields_for_cdl_v1() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    old_register = _mini_register(_PRE_330_CDL_V1_ROW)
    new_register = _mini_register(_register_row_text(rows["CDL-V1"]))
    assert_only_allowed_row_mutations(old_register, new_register, cdl_id="CDL-V1")


def test_non_target_rows_not_ratified_in_phase_330() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    assert_no_non_target_rows_marked_with_phase(
        rows,
        phase="330",
...
def test_full_non_target_row_mutation_guard_for_phase_330() -> None:
    commit_ref = _resolve_phase_330_commit_ref()
    ...
    target_old = _mini_register(_register_row_text(old_rows["CDL-V1"]))
    target_new = _mini_register(_register_row_text(new_rows["CDL-V1"]))
    assert_only_allowed_row_mutations(target_old, target_new, cdl_id="CDL-V1")

    for cdl_id in old_rows:
        if cdl_id == "CDL-V1":
            continue
        assert old_rows[cdl_id] == new_rows[cdl_id], f"row_{cdl_id}_unlawfully_mutated_in_phase_330"


def test_phase_330_commit_touched_no_runtime_files() -> None:
    commit_ref = _resolve_phase_330_commit_ref()
