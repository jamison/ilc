...
ARTIFACT_PATH = Path("docs/specs/ilc_phase_723_726_sequence_lock_v0.1.md")
TEST_PATH = Path("tests/test_phase_723_window_723_726_sequence_lock.py")
WALKTHROUGH_PATH = Path(
    "docs/phases/phase_723_g8_window_723_726_sequence_lock_walkthrough.md"
)
...
PHASE_MAIN_SUBJECT = ("phase 723", "window 723-726 sequence lock")
PHASE_BACKFILL_SUBJECT = ("phase 723", "walkthrough", "backfill")
...
def _resolve_commit_ref(subject_tokens: tuple[str, ...], expected_paths: set[str]) -> str:
    ...
    for commit_ref in matches:
        if _changed_paths_for_commit(commit_ref) == expected_paths:
            return commit_ref
    raise AssertionError("phase_723_commit_not_present_in_local_history")
...
def test_artifact_records_window_identity_and_baseline() -> None:
    text = _read(ARTIFACT_PATH)
    assert "Window `717-722` is closed. Capsule `v4.8` is the live main-lane frontier" in text
    assert "`CDL-062` remains open as the bounded\nsovereign-substrate research lane" in text
    assert "`M-015` complete and the next\nplanned M-phase as `M-016`" in text


def test_artifact_records_phase_table_and_closure_phase() -> None:
    text = _read(ARTIFACT_PATH)
    assert "| 1 | 723 | sequence lock |" in text
    assert "| 4 | 726 | closure |" in text
    assert "Phase `724` publishes the eligibility prefilter without activating the lane." in text
    assert "Closure phase is Phase `726`." in text


def test_artifact_records_separation_and_non_activation_boundaries() -> None:
    text = _read(ARTIFACT_PATH)
    assert "It is an eligibility / trigger window, not an\nactivation window." in text
    assert "`CDL-062` remains the sovereign-substrate research lane" in text
    assert "ADR-0022 and the private/gated shard header/capability-token lane remain" in text
    assert "does not treat historical “ADR-0018” references as accepted live\nADR law" in text
...
def test_phase_723_main_commit_touches_expected_paths_only() -> None:
    commit_ref = _resolve_commit_ref(PHASE_MAIN_SUBJECT, EXACT_REQUIRED_MAIN_PATHS)
    assert _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_MAIN_PATHS


def test_phase_723_backfill_commit_touches_walkthrough_and_status_only() -> None:
