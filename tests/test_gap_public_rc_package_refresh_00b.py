from __future__ import annotations

import json
from pathlib import Path

from ilc_core.release.install_sh_manifest_sync import verify_install_sh_manifest_sync
from ilc_core.release.installable_release_manifest import (
    validate_installable_release_manifest,
)
from ilc_core.release.installable_release_signature import validate_envelope_set


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_030 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_030_GAP_PUBLIC_RC_PACKAGE_REFRESH_00_v0.1.json"
)
MANIFEST_031 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_031_GAP_CDL017_PACKAGE_00a_v0.1.json"
)
MANIFEST_040 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_040_GAP_AGENT_ONBOARDING_PACKAGE_00a_v0.1.json"
)
MANIFEST_041 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_041_GAP_ONBOARDING_PACKAGE_FIX1_00a_v0.1.json"
)
MANIFEST_042 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_042_GAP_ONBOARDING_PACKAGE_042_00a_v0.1.json"
)
MANIFEST_043 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_043_GAP_ONBOARDING_PACKAGE_043_00a_v0.1.json"
)
MANIFEST_044 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_044_GAP_ONBOARDING_PACKAGE_044_00a_v0.1.json"
)
MANIFEST_045 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_045_GAP_CONNECTIVITY_PACKAGE_00a_v0.1.json"
)
MANIFEST_046 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_046_GAP_CONNECTIVITY_PACKAGE_00c_v0.1.json"
)
MANIFEST_047 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_047_GAP_RELAY_GUARD_ACTIVATION_BRIDGE_00_v0.1.json"
)
MANIFEST_048 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_048_GAP_RELAY_LIVE_UDP_DATAPLANE_FIX_00_v0.1.json"
)
MANIFEST_049 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_049_GAP_RELAY_RENDEZVOUS_DEPLOY_00_v0.1.json"
)
MANIFEST_0410 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0410_GAP_RELAY_AUDIT_HARDENING_PACKAGE_00b_v0.1.json"
)
MANIFEST_0411 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0411_GAP_INSTALL_CONNECTIVITY_PACKAGE_0411_00b_v0.1.json"
)
MANIFEST_0412 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0412_GAP_INSTALL_CONNECTIVITY_PACKAGE_0412_00b_v0.1.json"
)
MANIFEST_0413 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0413_GAP_RELAY_BOOTSTRAP_CAPSULE_SIGN_00_v0.1.json"
)
MANIFEST_0414 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0414_GAP_INSTALL_SHORTCODE_PACKAGE_00b_v0.1.json"
)
MANIFEST_0415 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0415_GAP_INVITE_SHORTCODE_DEPLOY_00_v0.1.json"
)
MANIFEST_0416 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0416_GAP_CONSENSUS_BINARY_DEPLOY_00_v0.1.json"
)
MANIFEST_0417 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0417_GAP_CONSENSUS_BINARY_DEPLOY_FIX2_00_v0.1.json"
)
MANIFEST_0418 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0418_GAP_PUBLIC_RC_INVITE_PACKAGE_0418_00b_v0.1.json"
)
MANIFEST_0419 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0419_GAP_PUBLIC_RC_INVITE_PACKAGE_0419_00b_v0.1.json"
)
MANIFEST_0420 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0420_GAP_PACKAGE_0420_00b_v0.1.json"
)
MANIFEST_0421 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0421_GAP_PACKAGE_0421_00b_v0.1.json"
)
MANIFEST_0422 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0422_GAP_PACKAGE_0422_00b_v0.1.json"
)
ENVELOPES_0422 = (
    ROOT / "docs/specs/ilc_core_0422_release_envelopes_GAP_PACKAGE_0422_00b_v0.1.json"
)
MANIFEST_0423 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0423_GAP_PACKAGE_0423_00b_v0.1.json"
)
ENVELOPES_0423 = (
    ROOT / "docs/specs/ilc_core_0423_release_envelopes_GAP_PACKAGE_0423_00b_v0.1.json"
)
MANIFEST_0424 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0424_GAP_PACKAGE_0424_00b_v0.1.json"
)
ENVELOPES_0424 = (
    ROOT / "docs/specs/ilc_core_0424_release_envelopes_GAP_PACKAGE_0424_00b_v0.1.json"
)
MANIFEST_0426 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0426_GAP_PACKAGE_0426_00b_v0.1.json"
)
ENVELOPES_0426 = (
    ROOT / "docs/specs/ilc_core_0426_release_envelopes_GAP_PACKAGE_0426_00b_v0.1.json"
)
MANIFEST_0427 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0427_GAP_PACKAGE_0427_00b_v0.1.json"
)
ENVELOPES_0427 = (
    ROOT / "docs/specs/ilc_core_0427_release_envelopes_GAP_PACKAGE_0427_00b_v0.1.json"
)
MANIFEST_0428 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0428_GAP_PACKAGE_0428_00b_v0.1.json"
)
ENVELOPES_0428 = (
    ROOT / "docs/specs/ilc_core_0428_release_envelopes_GAP_PACKAGE_0428_00b_v0.1.json"
)
MANIFEST_0429 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_0429_GAP_PACKAGE_0429_00b_v0.1.json"
)
ENVELOPES_0429 = (
    ROOT / "docs/specs/ilc_core_0429_release_envelopes_GAP_PACKAGE_0429_00b_v0.1.json"
)
MANIFEST_020 = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_020_GAP_PUBLIC_INSTALL_01_v0.1.json"
)
BUILD_RECEIPT = (
    ROOT / "docs/specs/ilc_package_build_receipt_GAP_PUBLIC_RC_PACKAGE_REFRESH_00a_v0.1.json"
)
UPLOAD_RECEIPT = (
    ROOT / "docs/specs/ilc_pypi_upload_receipt_GAP_PUBLIC_RC_PACKAGE_REFRESH_00b_v0.1.json"
)
SUPERSESSION_NOTE = (
    ROOT
    / "docs/specs/ilc_installable_release_manifest_ilc_core_020_superseded_by_030_GAP_PUBLIC_RC_PACKAGE_REFRESH_00b_v0.1.md"
)
INSTALL_SH = ROOT / "tools/install.sh"


def _load_json(path: Path) -> dict:
    with path.open(encoding="utf-8") as f:
        return json.load(f)


def test_manifest_030_validates_and_targets_ilc_core_030() -> None:
    manifest = _load_json(MANIFEST_030)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.3.0"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_030_uses_canonical_pypi_urls_and_unsigned_status() -> None:
    manifest = _load_json(MANIFEST_030)

    for artifact in manifest["artifacts"]:
        assert artifact["download_url"].startswith("https://files.pythonhosted.org/")
        assert "ilc_core-0.3.0" in artifact["download_url"]
        assert artifact["signing_status"] == "unsigned"


def test_build_receipt_and_upload_receipt_hashes_match_manifest() -> None:
    manifest = _load_json(MANIFEST_030)
    build_receipt = _load_json(BUILD_RECEIPT)
    upload_receipt = _load_json(UPLOAD_RECEIPT)
    wheel = next(
        artifact
        for artifact in manifest["artifacts"]
        if artifact["artifact_type"] == "python_wheel"
    )
    sdist = next(
        artifact
        for artifact in manifest["artifacts"]
        if artifact["artifact_type"] == "python_sdist"
    )

    assert build_receipt["pypi_upload_status"] == "uploaded"
    assert upload_receipt["pypi_upload_status"] == "uploaded"
    assert wheel["canonical_hash"] == f"sha256:{build_receipt['wheel_sha256']}"
    assert sdist["canonical_hash"] == f"sha256:{build_receipt['sdist_sha256']}"
    assert upload_receipt["wheel_sha256"] == build_receipt["wheel_sha256"]
    assert upload_receipt["sdist_sha256"] == build_receipt["sdist_sha256"]


def test_manifest_031_validates_and_targets_ilc_core_031() -> None:
    manifest = _load_json(MANIFEST_031)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.3.1"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_031_remains_valid_after_040_supersession() -> None:
    validate_installable_release_manifest(_load_json(MANIFEST_031))


def test_manifest_041_validates_and_targets_ilc_core_041() -> None:
    manifest = _load_json(MANIFEST_041)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.1"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_042_validates_and_targets_ilc_core_042() -> None:
    manifest = _load_json(MANIFEST_042)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.2"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_043_validates_and_targets_ilc_core_043() -> None:
    manifest = _load_json(MANIFEST_043)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.3"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_044_validates_and_targets_ilc_core_044() -> None:
    manifest = _load_json(MANIFEST_044)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.4"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_045_validates_and_targets_ilc_core_045() -> None:
    manifest = _load_json(MANIFEST_045)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.5"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_046_validates_and_targets_ilc_core_046() -> None:
    manifest = _load_json(MANIFEST_046)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.6"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_047_validates_and_targets_ilc_core_047() -> None:
    manifest = _load_json(MANIFEST_047)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.7"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_048_validates_and_targets_ilc_core_048() -> None:
    manifest = _load_json(MANIFEST_048)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.8"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_049_validates_and_targets_ilc_core_049() -> None:
    manifest = _load_json(MANIFEST_049)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.9"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0410_validates_and_targets_ilc_core_0410() -> None:
    manifest = _load_json(MANIFEST_0410)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.10"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0411_validates_and_targets_ilc_core_0411() -> None:
    manifest = _load_json(MANIFEST_0411)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.11"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0412_validates_and_targets_ilc_core_0412() -> None:
    manifest = _load_json(MANIFEST_0412)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.12"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0413_validates_and_targets_ilc_core_0413() -> None:
    manifest = _load_json(MANIFEST_0413)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.13"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0414_validates_and_targets_ilc_core_0414() -> None:
    manifest = _load_json(MANIFEST_0414)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.14"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0415_validates_and_targets_ilc_core_0415() -> None:
    manifest = _load_json(MANIFEST_0415)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.15"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0416_validates_and_targets_ilc_core_0416() -> None:
    manifest = _load_json(MANIFEST_0416)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.16"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0417_validates_and_targets_ilc_core_0417() -> None:
    manifest = _load_json(MANIFEST_0417)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.17"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0418_validates_and_targets_ilc_core_0418() -> None:
    manifest = _load_json(MANIFEST_0418)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.18"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0419_validates_and_targets_ilc_core_0419() -> None:
    manifest = _load_json(MANIFEST_0419)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.19"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0420_validates_and_targets_ilc_core_0420() -> None:
    manifest = _load_json(MANIFEST_0420)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.20"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0421_validates_and_targets_ilc_core_0421() -> None:
    manifest = _load_json(MANIFEST_0421)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.21"
    assert manifest["channel"] == "rc"
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0422_validates_and_targets_ilc_core_0422() -> None:
    manifest = _load_json(MANIFEST_0422)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.22"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0422_release_envelopes_GAP_PACKAGE_0422_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0422_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0422)
    envelopes = _load_json(ENVELOPES_0422)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0423_validates_and_targets_ilc_core_0423() -> None:
    manifest = _load_json(MANIFEST_0423)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.23"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0423_release_envelopes_GAP_PACKAGE_0423_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0423_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0423)
    envelopes = _load_json(ENVELOPES_0423)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0424_validates_and_targets_ilc_core_0424() -> None:
    manifest = _load_json(MANIFEST_0424)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.24"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0424_release_envelopes_GAP_PACKAGE_0424_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0424_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0424)
    envelopes = _load_json(ENVELOPES_0424)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0426_validates_and_targets_ilc_core_0426() -> None:
    manifest = _load_json(MANIFEST_0426)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.26"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0426_release_envelopes_GAP_PACKAGE_0426_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0426_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0426)
    envelopes = _load_json(ENVELOPES_0426)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0427_validates_and_targets_ilc_core_0427() -> None:
    manifest = _load_json(MANIFEST_0427)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.27"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0427_release_envelopes_GAP_PACKAGE_0427_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0427_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0427)
    envelopes = _load_json(ENVELOPES_0427)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0428_validates_and_targets_ilc_core_0428() -> None:
    manifest = _load_json(MANIFEST_0428)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.28"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0428_release_envelopes_GAP_PACKAGE_0428_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0428_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0428)
    envelopes = _load_json(ENVELOPES_0428)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_0429_validates_and_targets_ilc_core_0429() -> None:
    manifest = _load_json(MANIFEST_0429)

    validate_installable_release_manifest(manifest)

    assert manifest["release_id"] == "ilc-core-0.4.29"
    assert manifest["channel"] == "rc"
    assert manifest["release_envelope_ref"] == (
        "https://raw.githubusercontent.com/jamison/ilc/main/docs/specs/"
        "ilc_core_0429_release_envelopes_GAP_PACKAGE_0429_00b_v0.1.json"
    )
    assert {artifact["artifact_type"] for artifact in manifest["artifacts"]} == {
        "cli_binary",
        "python_sdist",
        "python_wheel",
    }


def test_manifest_0429_release_envelopes_cover_signed_artifacts() -> None:
    manifest = _load_json(MANIFEST_0429)
    envelopes = _load_json(ENVELOPES_0429)

    validate_envelope_set(envelopes, manifest=manifest)

    signed_artifact_ids = {
        artifact["artifact_id"]
        for artifact in manifest["artifacts"]
        if artifact["signing_status"] == "signed"
    }
    assert set(envelopes["envelopes"]) == signed_artifact_ids


def test_manifest_040_remains_valid_after_041_supersession() -> None:
    validate_installable_release_manifest(_load_json(MANIFEST_040))


def test_manifest_041_remains_valid_after_042_supersession() -> None:
    validate_installable_release_manifest(_load_json(MANIFEST_041))


def test_install_sh_is_synced_to_current_0429_manifest() -> None:
    verify_install_sh_manifest_sync(INSTALL_SH, MANIFEST_0429)


def test_old_manifest_still_valid_and_unmodified_by_supersession() -> None:
    validate_installable_release_manifest(_load_json(MANIFEST_020))
    old_manifest_text = MANIFEST_020.read_text(encoding="utf-8")

    assert "superseded_by" not in old_manifest_text


def test_supersession_note_names_old_new_and_upload_receipt() -> None:
    text = SUPERSESSION_NOTE.read_text(encoding="utf-8")

    assert MANIFEST_020.name in text
    assert MANIFEST_030.name in text
    assert UPLOAD_RECEIPT.name in text
    assert "No release signature was produced in this phase." in text
