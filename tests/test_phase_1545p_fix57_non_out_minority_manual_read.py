...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["assisted_triage_entries"] == 0
    assert data["pending_deep_manual_confirmation_entries"] == 0
    assert data["pending_entries"] == 0
    assert data["full_non_out_pass_status"] == "deep_manual_confirmation_complete"
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert pending == []
    assert data["full_pass_summary"]["file_direct_reads"] == 344
    ...
    assert len(pending_assisted) == 0
    assert len(data["deep_manual_confirmation_batches"]) == 45
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_046_final_infrastructure_authority_misc_nodes"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `432`" in report
    assert "Assisted-triage entries pending deep confirmation: `0`" in report
    assert "Total support-only edge recommendations currently recorded: `1903`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `432`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `0`" in walkthrough
...
def test_fix57_non_out_status_tokens() -> None:
    ...
    assert "fix57_non_out_deep_manual_confirmation_complete" in status
