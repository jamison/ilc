...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 377
    assert data["assisted_triage_entries"] == 55
    assert data["pending_deep_manual_confirmation_entries"] == 55
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert len(pending_assisted) == 55
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 39
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_040_authority_policy_invariant_gap"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `377`" in report
    assert "Assisted-triage entries pending deep confirmation: `55`" in report
    assert "Total support-only edge recommendations currently recorded: `1644`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `377`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `55`" in walkthrough
