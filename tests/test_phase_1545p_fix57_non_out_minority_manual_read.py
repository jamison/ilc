...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 397
    assert data["assisted_triage_entries"] == 35
    assert data["pending_deep_manual_confirmation_entries"] == 35
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert pending == []
    assert data["full_pass_summary"]["file_direct_reads"] == 319
    assert data["full_pass_summary"]["semantic_node_classifications"] == 113
    ...
    assert len(pending_assisted) == 35
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 41
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_042_sim_spectral_followon_semantic_nodes"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `397`" in report
    assert "Assisted-triage entries pending deep confirmation: `35`" in report
    assert "Total support-only edge recommendations currently recorded: `1737`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `397`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `35`" in walkthrough
