...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 199
    assert data["assisted_triage_entries"] == 233
    assert data["pending_deep_manual_confirmation_entries"] == 233
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 73
    ...
    assert len(pending_assisted) == 233
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 19
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_020_tests_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `199`" in report
    assert "Assisted-triage entries pending deep confirmation: `233`" in report
    assert "Total support-only edge recommendations currently recorded: `911`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `199`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `233`" in walkthrough
