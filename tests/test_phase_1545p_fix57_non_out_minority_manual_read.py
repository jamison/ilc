...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 221
    assert data["assisted_triage_entries"] == 211
    assert data["pending_deep_manual_confirmation_entries"] == 211
...
def test_fix57_non_out_edges_are_support_only_and_marked_by_confidence_boundary() -> None:
    ...
    assert len(confirmed) == 221
    assert len(assisted) == 211
    recommended_edges = [edge for row in data["entries"] for edge in row["recommended_edges"]]
    assert len(recommended_edges) == 1020
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert len(pending_assisted) == 211
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 22
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_023_tests_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `221`" in report
    assert "Assisted-triage entries pending deep confirmation: `211`" in report
    assert "Total support-only edge recommendations currently recorded: `1020`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `221`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `211`" in walkthrough
