...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 407
    assert data["assisted_triage_entries"] == 25
    assert data["pending_deep_manual_confirmation_entries"] == 25
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert pending == []
    assert data["full_pass_summary"]["file_direct_reads"] == 329
    assert data["full_pass_summary"]["semantic_node_classifications"] == 103
    ...
    assert len(pending_assisted) == 25
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 42
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_043_sim_remaining_semantic_nodes"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `407`" in report
    assert "Assisted-triage entries pending deep confirmation: `25`" in report
    assert "Total support-only edge recommendations currently recorded: `1795`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `407`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `25`" in walkthrough
