...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 209
    assert data["assisted_triage_entries"] == 223
    assert data["pending_deep_manual_confirmation_entries"] == 223
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 71
    ...
    assert len(pending_assisted) == 223
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 20
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_021_tests_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `209`" in report
    assert "Assisted-triage entries pending deep confirmation: `223`" in report
    assert "Total support-only edge recommendations currently recorded: `965`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `209`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `223`" in walkthrough
