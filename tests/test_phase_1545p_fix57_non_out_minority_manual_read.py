...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 327
    assert data["assisted_triage_entries"] == 105
    assert data["pending_deep_manual_confirmation_entries"] == 105
...
def test_fix57_non_out_edges_are_support_only_and_marked_by_confidence_boundary() -> None:
    ...
    assert len(confirmed) == 327
    assert len(assisted) == 105
    recommended_edges = [edge for row in data["entries"] for edge in row["recommended_edges"]]
    assert len(recommended_edges) == 1408
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert len(pending_assisted) == 105
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 34
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_035_artifact_target_diagnostic_gap"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `327`" in report
    assert "Assisted-triage entries pending deep confirmation: `105`" in report
    assert "Total support-only edge recommendations currently recorded: `1408`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `327`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `105`" in walkthrough
