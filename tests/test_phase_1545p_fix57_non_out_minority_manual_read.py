...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 417
    assert data["assisted_triage_entries"] == 15
    assert data["pending_deep_manual_confirmation_entries"] == 15
    assert data["pending_entries"] == 0
    assert data["full_non_out_pass_status"] == "deep_manual_confirmation_in_progress"
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert pending == []
    assert data["full_pass_summary"]["file_direct_reads"] == 339
    assert data["full_pass_summary"]["semantic_node_classifications"] == 93
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 42
    ...
    assert len(pending_assisted) == 15
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 43
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_044_phase_authority_policy_semantic_nodes"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `417`" in report
    assert "Assisted-triage entries pending deep confirmation: `15`" in report
    assert "Total support-only edge recommendations currently recorded: `1847`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `417`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `15`" in walkthrough
