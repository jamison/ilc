...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 169
    assert data["assisted_triage_entries"] == 263
    assert data["pending_deep_manual_confirmation_entries"] == 263
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 89
    ...
    assert len(pending_assisted) == 263
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 16
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_017_tests_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `169`" in report
    assert "Assisted-triage entries pending deep confirmation: `263`" in report
    assert "Total support-only edge recommendations currently recorded: `764`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `169`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `263`" in walkthrough
