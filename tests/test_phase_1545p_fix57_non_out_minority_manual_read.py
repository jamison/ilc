...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 271
    assert data["assisted_triage_entries"] == 161
    assert data["pending_deep_manual_confirmation_entries"] == 161
...
def test_fix57_non_out_edges_are_support_only_and_marked_by_confidence_boundary() -> None:
    ...
    assert len(confirmed) == 271
    assert len(assisted) == 161
    recommended_edges = [edge for row in data["entries"] for edge in row["recommended_edges"]]
    assert len(recommended_edges) == 1191
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert pending == []
    assert data["full_pass_summary"]["file_direct_reads"] == 308
    assert data["full_pass_summary"]["semantic_node_classifications"] == 124
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 59
    ...
    assert len(pending_assisted) == 161
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 28
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_029_tools_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `271`" in report
    assert "Assisted-triage entries pending deep confirmation: `161`" in report
    assert "Total support-only edge recommendations currently recorded: `1191`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `271`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `161`" in walkthrough
