...
def test_fix57_non_out_ledger_counts_and_deferred_raw_out_boundary() -> None:
    ...
    assert data["manual_reviewed_entries"] == 189
    assert data["assisted_triage_entries"] == 243
    assert data["pending_deep_manual_confirmation_entries"] == 243
...
def test_fix57_non_out_all_assisted_rows_wait_for_deep_manual_confirmation() -> None:
    ...
    assert data["full_pass_summary"]["reviewed_with_no_new_safe_edge"] == 79
    ...
    assert len(pending_assisted) == 243
    ...
    assert len(data["deep_manual_confirmation_batches"]) == 18
    assert data["deep_manual_confirmation_batches"][-1]["batch_id"] == "batch_019_tests_other"
...
def test_fix57_non_out_report_and_walkthrough_record_full_pass() -> None:
    ...
    assert "Deep-manual confirmed entries: `189`" in report
    assert "Assisted-triage entries pending deep confirmation: `243`" in report
    assert "Total support-only edge recommendations currently recorded: `860`" in report
    assert "Honesty Correction" in report
    assert "Deep-manual confirmed rows | `189`" in walkthrough
    assert "Assisted-triage rows pending deep confirmation | `243`" in walkthrough
