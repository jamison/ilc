"""Phase 377 closure gate contract tests for window 368-377."""
...
GATE_PATH = Path("tools/check_window_368_377_closure_gate_phase_377.sh")
GATE_CMD = ["bash", str(GATE_PATH)]
HANDOFF_PATH = Path("docs/specs/ilc_window_368_377_handoff_377_v0.1.md")
SNAPSHOT_PATH = Path("out/monitoring/infrastructure_risk_snapshot_phase_316.json")
DECISION_LOG_PATH = "docs/specs/ilc_constitutional_decision_log_v0.1.md"
PHASE_377_COMMIT_SUBJECT = "docs(g8): phase 377 window 368-377 closure gate and 378-plus handoff"
...
def _snapshot_override_env(tmp_path: Path, verdict: str) -> tuple[dict[str, str], bytes, int, str]:
    ...
    env["ILC_PHASE_377_SNAPSHOT_PATH"] = str(snapshot_copy)
...
def _resolve_phase_377_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if subject.strip() == PHASE_377_COMMIT_SUBJECT:
    ...
    if matching_commits:
        raise AssertionError("phase_377_commit_subject_present_but_no_qualifying_closure_commit")
    raise AssertionError("phase_377_commit_not_present_in_local_history")
...
def test_gate_cli_contract_dry_run_help_alias_unknown_arg_and_exact_lines() -> None:
    ...
    assert lines == [
        ...
        "python3 tools/validate_phase_prompt.py docs/antigravity_tasks/antigravity_prompt__phase_377_g8_constitution_cluster_a_window_368_377_closure_verification_gate_and_378_plus_handoff.md",
        "[2/6] lane_contract_tests",
        "python3 -m pytest tests/test_phase_368_sequence_lock.py tests/test_phase_369_sim_004_commissioning.py tests/test_phase_370_sim_005_commissioning.py tests/test_phase_371_sim_interpretation_and_risk_closure.py tests/test_cdl_039_topology_privacy_hardening_prelock_372.py tests/test_cdl_039_adversarial_review_and_evidence_freeze_373.py tests/test_cdl_039_prelock_finalization_374.py tests/test_phase_375_v_series_implementation_window_sequence_lock.py tests/test_phase_376_coherence_and_capsule_v1_2.py -q",
        "[3/6] cross_phase_regression",
        "python3 -m pytest tests/test_phase_commit_manifest_296.py tests/test_d2_schema_baseline_runtime_310.py tests/test_genesis_state_bundle_runtime_312.py tests/test_epoch_snapshot_runtime_314.py tests/test_infrastructure_economic_risk_monitoring_update_315.py tests/test_infrastructure_composed_preflight_316.py tests/test_wire_transport_runtime_323.py tests/test_window_358_367_closure_gate_367.py tests/test_window_348_357_closure_gate_357.py tests/test_window_338_347_closure_gate_347.py -q",
        "[4/6] mutation_canary",
        "python3 tools/run_mutation_canary_phase_297.py",
        "[5/6] closure_gate_cli_contract",
        "python3 -m pytest tests/test_window_368_377_closure_gate_377.py -q",
...
def test_gate_full_run_exits_zero_on_pass_snapshot_and_preserves_canonical_snapshot() -> None:
    if os.environ.get("ILC_PHASE_377_GATE_SELFTEST") == "1":
        pytest.skip("phase_377_selftest_context_skip_full_gate")
    ...
    assert "phase_377_snapshot_gate=passed" in result.stdout
    assert "phase_377_verdict=pass" in result.stdout
...
def test_gate_full_run_enforces_conditional_exit_and_snapshot_isolation(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_377_GATE_SELFTEST") == "1":
        pytest.skip("phase_377_selftest_context_skip_verdict_simulation")
    env, canonical, canonical_mtime, canonical_sha = _snapshot_override_env(tmp_path, "conditional")
    result = _run_gate([], env=env)
    assert result.returncode == 3
    assert "phase_377_override_required=human" in result.stdout
    assert "phase_377_snapshot_gate=failed" in result.stdout
...
def test_no_decision_log_mutation_in_phase_commit() -> None:
    if os.environ.get("ILC_PHASE_377_GATE_SELFTEST") == "1":
        pytest.skip("phase_377_selftest_context_skip_commit_guardrails")
    commit_ref = _resolve_phase_377_commit_ref()
...
def test_no_ilc_core_runtime_feature_mutation_in_phase_commit() -> None:
    ...
    assert not forbidden, f"phase_377_runtime_feature_mutations:{forbidden}"
