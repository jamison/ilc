...
def _resolve_phase_576_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if PHASE_576_SUBJECT_TOKEN not in subject.lower():
            continue
        matches.append(commit_hash)
    for commit_ref in matches:
        if _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_MAIN_PATHS:
            return commit_ref
    raise AssertionError('phase_576_commit_not_present_in_local_history')
...
def test_phase_576_commit_touches_expected_paths_only() -> None:
    commit_ref = _resolve_phase_576_commit_ref()
    assert _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_MAIN_PATHS


def test_phase_576_commit_touches_no_cdl_path_and_no_ilc_core_path() -> None:
    commit_ref = _resolve_phase_576_commit_ref()
