"""Phase 433 closure gate contract tests for window 424-433."""
...
GATE_PATH = Path("tools/check_window_424_433_closure_gate_phase_433.sh")
GATE_CMD = ["bash", str(GATE_PATH)]
HANDOFF_PATH = Path("docs/specs/ilc_window_424_433_handoff_433_v0.1.md")
...
POPPER_RUNTIME_PATH = Path("ilc_core/consensus/popperian_gate_runtime.py")
PHASE_433_COMMIT_SUBJECT = "docs(g8): phase 433 window 424-433 closure gate and 434-plus handoff"
...
def _snapshot_override_env(tmp_path: Path, verdict: str) -> tuple[dict[str, str], bytes, int, str]:
    ...
    env["ILC_PHASE_433_SNAPSHOT_PATH"] = str(snapshot_copy)
...
def _resolve_phase_433_commit_ref() -> str:
    ...
    required_paths = {
        str(GATE_PATH),
        str(Path("tests/test_window_424_433_closure_gate_433.py")),
        str(HANDOFF_PATH),
    }
    ...
    for line in result.stdout.splitlines():
        ...
        if subject.strip() == PHASE_433_COMMIT_SUBJECT:
    ...
    if matching_commits:
        raise AssertionError("phase_433_commit_subject_present_but_no_qualifying_closure_commit")
    raise AssertionError("phase_433_commit_not_present_in_local_history")


def _admissible_claim_forms_block(text: str) -> str:
    match = re.search(r"_ADMISSIBLE_CLAIM_FORMS\s*=\s*\{(?P<body>.*?)\n\}", text, re.DOTALL)
    assert match, "phase_433_missing_admissible_claim_forms_block"
...
def test_gate_cli_contract_dry_run_help_alias_unknown_arg_and_exact_lines() -> None:
    ...
    assert lines == [
        ...
        "python3 tools/validate_phase_prompt.py docs/antigravity_tasks/antigravity_prompt__phase_433_g8_window_424_433_closure_gate_and_434_plus_handoff.md",
        "[2/6] lane_contract_tests",
        "python3 -m pytest tests/test_phase_424_cdl_049_opening.py tests/test_phase_425_cdl_049_bounded_existential_alignment_prelock_hardening.py tests/test_phase_426_treasury_pe_stabilization_governance_review.py tests/test_phase_427_cdl_049_ratification_evidence_assembly.py tests/test_phase_428_cdl_049_bounded_existential_alignment_ratification.py tests/test_phase_429_sim_009_pe_stabilization_commissioning.py tests/test_phase_430_sim_009_results_synthesis_and_disposition.py tests/test_phase_431_pe_stabilization_carry_forward_decision.py tests/test_phase_432_coherence_and_capsule_v1_7.py -q",
        ...
        "python3 -m pytest tests/test_window_424_433_closure_gate_433.py -q",
...
def test_gate_full_run_exits_zero_on_pass_snapshot_and_preserves_canonical_snapshot() -> None:
    if os.environ.get("ILC_PHASE_433_GATE_SELFTEST") == "1":
        pytest.skip("phase_433_selftest_context_skip_full_gate")
    ...
    assert "phase_433_snapshot_gate=passed" in result.stdout
    assert "phase_433_verdict=pass" in result.stdout
...
def test_gate_full_run_enforces_conditional_exit_and_snapshot_isolation(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_433_GATE_SELFTEST") == "1":
        pytest.skip("phase_433_selftest_context_skip_verdict_simulation")
    env, canonical, canonical_mtime, canonical_sha = _snapshot_override_env(tmp_path, "conditional")
    result = _run_gate([], env=env)
    assert result.returncode == 3
    assert "phase_433_override_required=human" in result.stdout
    assert "phase_433_snapshot_gate=failed" in result.stdout
...
def test_gate_assertions_and_handoff_contract_tokens() -> None:
    ...
    for token in ("ratified_phase", "ratified_date", "evidence_document"):
        assert token in row, f"phase_433_missing_{token}:CDL-049"
    assert "CDL-050" not in rows

    runtime_text = POPPER_RUNTIME_PATH.read_text(encoding="utf-8")
    admissible_block = _admissible_claim_forms_block(runtime_text)
    assert '"bounded_existential"' in admissible_block
    assert '"existential"' not in admissible_block

    popper_analysis_text = POPPER_ANALYSIS_PATH.read_text(encoding="utf-8")
    assert "bounded_existential" in popper_analysis_text
    assert not re.search(r"(?<!bounded_)\bexistential\b", popper_analysis_text)
    ...
    carry_forward_text = PE_CARRY_FORWARD_PATH.read_text(encoding="utf-8")
    assert "The P_e constitutional lane carries into Window 434+ as the default continuation target." in carry_forward_text

    coherence_text = COHERENCE_PATH.read_text(encoding="utf-8")
    assert "Phase 431 published the Treasury P_e carry-forward decision under Scenario B with Window 434+ as the default continuation target." in coherence_text
...
def test_no_decision_log_mutation_in_phase_commit() -> None:
    if os.environ.get("ILC_PHASE_433_GATE_SELFTEST") == "1":
        pytest.skip("phase_433_selftest_context_skip_commit_guardrails")
    commit_ref = _resolve_phase_433_commit_ref()
