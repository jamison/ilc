...
def test_all_constants_have_exact_expected_values() -> None:
    ...
    assert runtime.PASSIVE_ATTRIBUTION_RATE == 0.20
    assert runtime.DECAY_FLOOR == 0.05
    assert runtime.ATTRIBUTION_CAP == 0.15
...
def test_quality_factor_matches_expected_bounds_and_neutral_midpoint() -> None:
    assert runtime.quality_factor(0.0) == 0.85
    assert runtime.quality_factor(1.0) == 1.15
    assert runtime.quality_factor(0.5) == 1.0


def test_compute_passive_ecu_returns_zero_below_decay_floor() -> None:
    assert runtime.compute_passive_ecu(1.0, 0.04, 0.5) == 0.0


def test_compute_passive_ecu_returns_zero_for_zero_base_reward() -> None:
    assert runtime.compute_passive_ecu(0.0, 1.0, 0.5) == 0.0


def test_compute_passive_ecu_applies_output_layer_cap() -> None:
    assert runtime.compute_passive_ecu(1.0, 1.0, 1.0) == 0.15


def test_compute_passive_ecu_returns_uncapped_value_when_below_cap() -> None:
    assert runtime.compute_passive_ecu(1.0, 0.5, 0.5) == 0.10
