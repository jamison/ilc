...
HANDOFF_PATH = Path('docs/specs/ilc_window_525_534_handoff_534_v0.1.md')
GATE_PATH = Path('tools/check_window_525_534_closure_gate_phase_534.sh')
TEST_PATH = Path('tests/test_window_525_534_closure_gate_534.py')
PHASE_534_SUBJECT_TOKEN = 'phase 534 window 525-534 closure gate and handoff'
...
def _clean_gate_env() -> dict[str, str]:
    blocked = {
        ...
        'ILC_PHASE_534_DECISION_LOG_PATH',
        'ILC_PHASE_534_SYNTHESIS_PATH',
        'ILC_PHASE_534_RUNTIME_PATH',
        'ILC_PHASE_534_SNAPSHOT_PATH',
    }
...
def _resolve_phase_534_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if PHASE_534_SUBJECT_TOKEN not in subject.lower():
            continue
    ...
    for commit_ref in matching:
        if _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_MAIN_PATHS:
            return commit_ref
    if matching:
        raise AssertionError('phase_534_commit_subject_present_but_no_qualifying_closure_commit')
    raise AssertionError('phase_534_commit_not_present_in_local_history')
...
def _blocked_override_env(tmp_path: Path) -> dict[str, str]:
    synthesis_text = SYNTHESIS_PATH.read_text(encoding='utf-8').replace('cdl_059_opening_authorized', 'cdl_059_opening_deferred')
    synthesis_override = tmp_path / 'phase_534_synthesis_override.md'
    synthesis_override.write_text(synthesis_text, encoding='utf-8')

    lines = []
    for line in DECISION_LOG_PATH.read_text(encoding='utf-8').splitlines():
        if line.startswith('| CDL-059 |'):
            continue
        lines.append(line)
    decision_override = tmp_path / 'phase_534_decision_log_override.md'
    decision_override.write_text('\n'.join(lines) + '\n', encoding='utf-8')

    env = _clean_gate_env()
    env['ILC_PHASE_534_SYNTHESIS_PATH'] = str(synthesis_override)
    env['ILC_PHASE_534_DECISION_LOG_PATH'] = str(decision_override)
    env['ILC_PHASE_534_RUNTIME_PATH'] = str(tmp_path / 'missing_aesthetic_panel_runtime.py')
    env['ILC_PHASE_534_SNAPSHOT_PATH'] = str(tmp_path / 'phase_534_snapshot.json')
    return env
...
def test_gate_script_exists_is_executable_and_dry_run_matches_expected_output() -> None:
    ...
    assert len(EXPECTED_SELFTEST_GUARDS) == 24
...
def test_window_handoff_artifact_exists_and_contains_required_headings_and_tokens() -> None:
    ...
    for heading in (
        ...
        '## 3. Aesthetic panel governance outcome',
        '## 4. Runtime outcome',
    ...
    for token in (
        ...
        'Phase 535+ requires a new sequence lock or amendment.',
        'CDL-036 gossip schema amendment is a Window 535+ carry-forward.',
        'CDL-059 aesthetic panel governance is ratified in Phase 531.',
        'Aesthetic panel runtime is implemented in Phase 532.',
        'ADR-0023 simulation evidence chain is complete through Phase 527.',
    ):
        assert token in text


def test_gate_runs_success_path_and_preserves_canonical_monitoring(tmp_path: Path) -> None:
    if os.environ.get('ILC_PHASE_534_GATE_SELFTEST') == '1':
        pytest.skip('phase_534_selftest_context_skip_full_gate')
    before = _monitoring_state()
    env = _clean_gate_env()
    env['ILC_PHASE_534_SNAPSHOT_PATH'] = str(tmp_path / 'phase_534_snapshot.json')
    result = _run_gate([], env=env)
    assert result.returncode == 0
    assert 'phase_534_window_state=success_path' in result.stdout
    assert 'phase_534_verdict=pass' in result.stdout
    assert (tmp_path / 'phase_534_snapshot.json').exists()
...
def test_gate_accepts_blocked_override_and_passes(tmp_path: Path) -> None:
    if os.environ.get('ILC_PHASE_534_GATE_SELFTEST') == '1':
        pytest.skip('phase_534_selftest_context_skip_full_gate')
    env = _blocked_override_env(tmp_path)
    result = _run_gate([], env=env)
    assert result.returncode == 0
    assert 'phase_534_window_state=blocked_path' in result.stdout
    assert 'phase_534_verdict=pass' in result.stdout
    snapshot_text = (tmp_path / 'phase_534_snapshot.json').read_text(encoding='utf-8')
    assert '"state": "blocked"' in snapshot_text


def test_gate_rejects_invalid_when_runtime_is_absent(tmp_path: Path) -> None:
    if os.environ.get('ILC_PHASE_534_GATE_SELFTEST') == '1':
        pytest.skip('phase_534_selftest_context_skip_full_gate')
    env = _clean_gate_env()
    env['ILC_PHASE_534_RUNTIME_PATH'] = str(tmp_path / 'missing_aesthetic_panel_runtime.py')
    ...
    assert 'phase_534_window_state=invalid' in result.stdout
    assert 'phase_534_window_state_details=cdl_059_ratified_but_runtime_missing' in result.stdout


def test_cdl_inventory_is_historicalized_for_phase_534() -> None:
    if os.environ.get('ILC_PHASE_534_GATE_SELFTEST') == '1':
        pytest.skip('phase_534_selftest_context_skip_inventory')
    commit_ref = _resolve_phase_534_commit_ref()
    text = subprocess.run(
...
def test_phase_534_main_commit_touches_expected_paths_only() -> None:
    if os.environ.get('ILC_PHASE_534_GATE_SELFTEST') == '1':
        pytest.skip('phase_534_selftest_context_skip_commit_scope')
    commit_ref = _resolve_phase_534_commit_ref()
