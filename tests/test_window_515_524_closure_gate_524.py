...
HANDOFF_PATH = Path("docs/specs/ilc_window_515_524_handoff_524_v0.1.md")
GATE_PATH = Path("tools/check_window_515_524_closure_gate_phase_524.sh")
TEST_PATH = Path("tests/test_window_515_524_closure_gate_524.py")
PHASE_524_SUBJECT_TOKEN = "phase 524 window 515-524 closure gate and handoff"
...
def _clean_gate_env() -> dict[str, str]:
    blocked = {
        ...
        "ILC_PHASE_524_DECISION_LOG_PATH",
        "ILC_PHASE_524_EPOCH_RUNTIME_PATH",
        "ILC_PHASE_524_READMISSION_RUNTIME_PATH",
        "ILC_PHASE_524_SNAPSHOT_PATH",
    }
...
def _resolve_phase_524_commit_ref() -> str:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if PHASE_524_SUBJECT_TOKEN not in subject.lower():
            continue
    ...
    for commit_ref in matching:
        if _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_MAIN_PATHS:
            return commit_ref
    if matching:
        raise AssertionError("phase_524_commit_subject_present_but_no_qualifying_closure_commit")
    raise AssertionError("phase_524_commit_not_present_in_local_history")
...
def _decision_log_override_env(tmp_path: Path, scenario: str) -> dict[str, str]:
    ...
    if scenario == "blocked_open_row":
        ...
        for line in text.splitlines():
            if not line.startswith("| CDL-058 |"):
                updated.append(line)
                continue
            cells = [cell.strip() for cell in line.strip().split("|")[1:-1]]
            cells[3] = "open"
            updated.append("| " + " | ".join(cells[:7]) + " |")
            found = True
        assert found
        text = "\n".join(updated) + "\n"
    else:
        raise AssertionError(f"unknown_override_scenario:{scenario}")
    override_path = tmp_path / "phase_524_decision_log_override.md"
    override_path.write_text(text, encoding="utf-8")
    snapshot_path = tmp_path / "phase_524_snapshot.json"
    env = _clean_gate_env()
    env["ILC_PHASE_524_DECISION_LOG_PATH"] = str(override_path)
    env["ILC_PHASE_524_READMISSION_RUNTIME_PATH"] = str(tmp_path / "missing_re_admission_runtime.py")
    env["ILC_PHASE_524_SNAPSHOT_PATH"] = str(snapshot_path)
    return env
...
def test_gate_script_exists_is_executable_and_dry_run_matches_expected_output() -> None:
    ...
    assert len(EXPECTED_SELFTEST_GUARDS) == 23
...
def test_gate_runs_success_path_and_preserves_canonical_monitoring(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_524_GATE_SELFTEST") == "1":
        pytest.skip("phase_524_selftest_context_skip_full_gate")
    before = _monitoring_state()
    env = _clean_gate_env()
    env["ILC_PHASE_524_SNAPSHOT_PATH"] = str(tmp_path / "phase_524_snapshot.json")
    result = _run_gate([], env=env)
    assert result.returncode == 0
    assert "phase_524_window_state=success_path" in result.stdout
    assert "phase_524_verdict=pass" in result.stdout
    assert (tmp_path / "phase_524_snapshot.json").exists()
...
def test_gate_accepts_blocked_override_and_passes(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_524_GATE_SELFTEST") == "1":
        pytest.skip("phase_524_selftest_context_skip_full_gate")
    env = _decision_log_override_env(tmp_path, "blocked_open_row")
    result = _run_gate([], env=env)
    assert result.returncode == 0
    assert "phase_524_window_state=blocked_path" in result.stdout
    assert "phase_524_verdict=pass" in result.stdout
    snapshot_text = (tmp_path / "phase_524_snapshot.json").read_text(encoding="utf-8")
    assert '"state": "blocked"' in snapshot_text


def test_gate_rejects_invalid_when_epoch_runtime_is_absent(tmp_path: Path) -> None:
    if os.environ.get("ILC_PHASE_524_GATE_SELFTEST") == "1":
        pytest.skip("phase_524_selftest_context_skip_full_gate")
    env = _clean_gate_env()
    env["ILC_PHASE_524_EPOCH_RUNTIME_PATH"] = str(tmp_path / "missing_epoch_runtime.py")
    env["ILC_PHASE_524_SNAPSHOT_PATH"] = str(tmp_path / "phase_524_snapshot.json")
    result = _run_gate([], env=env)
    assert result.returncode == 1
    assert "phase_524_window_state=invalid" in result.stdout
    assert "phase_524_window_state_details=cdl_057_runtime_missing" in result.stdout


def test_cdl_inventory_is_historicalized_for_phase_524() -> None:
    if os.environ.get("ILC_PHASE_524_GATE_SELFTEST") == "1":
        pytest.skip("phase_524_selftest_context_skip_inventory")
    commit_ref = _resolve_phase_524_commit_ref()
    result = subprocess.run(
    ...
    assert rows.get("CDL-058", {}).get("status") in {"ratified", "open", None}
    assert "CDL-053" not in rows
    assert "CDL-059" not in rows


def test_phase_524_main_commit_touches_expected_paths_only_and_no_cdl_or_ilc_core() -> None:
    if os.environ.get("ILC_PHASE_524_GATE_SELFTEST") == "1":
        pytest.skip("phase_524_selftest_context_skip_commit_resolution")
    commit_ref = _resolve_phase_524_commit_ref()
