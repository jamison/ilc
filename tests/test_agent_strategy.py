...
def test_decide_stake_respects_ecu_fee_and_wallet():
    ...
    agent.wallet_balance = 1.0  # small but non-zero
    ...
    chosen = agent.decide_stake_for_claim(requested_stake=fee / 10.0)
    assert chosen >= fee
    ...
    chosen3 = agent.decide_stake_for_claim(requested_stake=agent.wallet_balance * 10.0)
...
def test_mine_thought_uses_strategy_and_succeeds_with_funds():
    ...
    thought = agent.mine_thought("2 < 3", parent, stake=1.0)
