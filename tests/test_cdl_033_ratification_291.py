...
PHASE_291_COMMIT_SUBJECT = "docs(g8): phase 291 cdl-033 ratification"
...
def _resolve_phase_291_commit_ref() -> str:
    ...
    for line in result.stdout.splitlines():
        ...
        if subject.strip() == PHASE_291_COMMIT_SUBJECT:
            return commit_hash
    pytest.skip("phase_291_commit_subject_not_found: commit-scoped assertions skipped")
...
def test_non_target_row_not_ratified_in_phase_291() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    assert rows["CDL-024"].get("ratified_phase") != "291"
