...
def test_build_message_uses_canonical_type_and_schema() -> None:
    message = build_nullifier_gossip_message(NULLIFIER, claimed_actor="agent:inviter")

    assert message == {
        "claimed_actor": "agent:inviter",
...
def test_uppercase_nullifier_rejected() -> None:
    with pytest.raises(InviteNullifierError, match="invite_nullifier_gossip_invalid_hex"):
        build_nullifier_gossip_message("A" * 64)


def test_unknown_key_rejected() -> None:
    message = build_nullifier_gossip_message(NULLIFIER)
...
def test_handle_registers_incoming_nullifier_once() -> None:
    ...
    handle_nullifier_gossip_message(message, registry)
    handle_nullifier_gossip_message(message, registry)
...
def test_cross_node_gossip_blocks_redeemed_invite_when_enforcement_enabled(
    ...
    message = build_nullifier_gossip_message(redemption.redemption_nullifier)
...
def test_openclaw_broadcasts_after_local_registration(tmp_path: Path) -> None:
    ...
    nullifier = derive_redemption_nullifier("openclaw-fix2d-batch", str(bundle["private_invite_nonce"]))
    ...
    assert messages == [build_nullifier_gossip_message(nullifier)]
