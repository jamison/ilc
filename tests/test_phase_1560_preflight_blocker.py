...
def test_sequence_lock_marks_phase_1560_blocked_precondition() -> None:
    ...
    assert "Phase 1560 preflight blocker recorded" in lock
    assert "BLOCKED_PRECONDITION - preflight blocker recorded" in lock
...
def test_status_and_planning_index_frontier_updated_to_blocker() -> None:
    ...
    assert "Phase 1560 preflight blocker" in index[:1200]
    assert "phase_1560_preflight_blocker_recorded" in index[:1600]
    assert "⬅ CURRENT" in index[:1600]
