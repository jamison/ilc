...
PHASE_272_COMMIT_SUBJECT = "docs(g8): phase 272 cdl-029 allocation split ratification"
...
_PRE_272_CDL_029_ROW = (
    "| CDL-029 | CDL-005 / CDL-011 | Allocation split (performer/auditor/genesis) validation and lock | "
    "open | confirm 80/15/5, revise split, redesign | 80/15/5 proposed — requires `theta_hard = 1/20` "
    "validation | allocation validation tests, governance spec alignment |"
)
...
def _resolve_phase_272_commit_ref() -> str:
...
def test_ratification_evidence_file_exists_and_has_required_content() -> None:
    ...
    assert "80 / 15 / 5" in text or "80/15/5" in text
    assert "theta_hard = 1/20" in text


def test_decision_log_cdl_029_is_ratified_with_expected_metadata() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    row = rows["CDL-029"]
    assert row["status"] == "ratified"
    assert row["ratified_phase"] == "272"
...
def test_mutation_scope_guardrail_allows_only_ratification_fields_for_cdl_029() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    old_register = _mini_register(_PRE_272_CDL_029_ROW)
    new_register = _mini_register(_register_row_text(rows["CDL-029"]))

    assert_only_allowed_row_mutations(old_register, new_register, cdl_id="CDL-029")


def test_previously_ratified_rows_remain_ratified() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    for cdl_id in ["CDL-001", "CDL-002", "CDL-007", "CDL-019", "CDL-025", "CDL-032"]:
        assert rows[cdl_id]["status"] == "ratified", cdl_id


def test_non_target_issuance_rows_not_ratified_in_phase_272() -> None:
    rows = parse_decision_register_rows(_read(DECISION_LOG_PATH))
    for cdl_id in ["CDL-026", "CDL-027", "CDL-028", "CDL-030", "CDL-031"]:
        assert rows[cdl_id].get("ratified_phase") != "272", cdl_id


def test_phase_272_commit_touched_no_runtime_files() -> None:
    commit_ref = _resolve_phase_272_commit_ref()
