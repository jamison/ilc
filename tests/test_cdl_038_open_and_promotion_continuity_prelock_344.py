...
# The Phase-344 `CDL-038` row is a historical prelock reference.
...
def test_decision_log_contains_exact_new_row_and_open_state() -> None:
    historical_text = _decision_log_text_at_ref(_resolve_phase_344_commit_ref())
    assert EXPECTED_ROW in historical_text
    historical_rows = parse_decision_register_rows(historical_text)
    row = historical_rows["CDL-038"]
...
def test_mutation_scope_for_cdl_038_is_additive_only() -> None:
    historical_rows = parse_decision_register_rows(
        _decision_log_text_at_ref(_resolve_phase_344_commit_ref())
    )
