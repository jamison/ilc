from __future__ import annotations

import re
import subprocess
from pathlib import Path

from ilc_core.testing.ratification_mutation_scope_guardrail import (
    assert_head_commit_touched_no_runtime_files,
    parse_decision_register_rows,
)
from simulations import sim_treasury_scenario5_waggle_dance_recovery_rule as nonlinear_module

MODULE_PATH = Path('simulations/sim_treasury_scenario5_waggle_dance_recovery_rule.py')
ATTESTATION_PATH = Path('docs/specs/ilc_phase_456_fix_10_nonlinear_control_mechanism_surface_attestation_v0.1.md')
TEST_PATH = Path('tests/test_phase_456_fix_10_nonlinear_control_mechanism_implementation.py')
WALKTHROUGH_PATH = Path('docs/phases/phase_456_fix_10_g8_nonlinear_control_mechanism_implementation_walkthrough.md')
STATUS_PATH = Path('docs/phases/STATUS.md')
DECISION_LOG_PATH = Path('docs/specs/ilc_constitutional_decision_log_v0.1.md')
PHASE_SUBJECT = 'feat(simulations): phase 456 fix 10 nonlinear-control mechanism surface implementation'
FORBIDDEN_TREASURY_TOKEN = 'ILC_CDL_MUTATION_' + 'AUTHORIZED'
EXACT_REQUIRED_PATHS = {
    str(MODULE_PATH),
    str(ATTESTATION_PATH),
    str(TEST_PATH),
    str(WALKTHROUGH_PATH),
    str(STATUS_PATH),
}
EXPECTED_CANDIDATES = {
    'nonlinear_curve_k56_g18_f22_frontier_first',
    'nonlinear_curve_k58_g18_f22_frontier_first',
    'nonlinear_curve_k60_g18_f22_frontier_first',
    'mixed_queue_and_production',
}


def _read(path: Path) -> str:
    return path.read_text(encoding='utf-8')


def _section_body(text: str, heading: str) -> str:
    escaped = re.escape(heading)
    match = re.search(rf'{escaped}\n\n(.*?)(?=\n## |\Z)', text, flags=re.S)
    if not match:
        raise AssertionError(f'section_not_found:{heading}')
    return match.group(1).strip()


def _changed_paths_for_commit(commit_ref: str) -> set[str]:
    result = subprocess.run(['git', 'show', '--name-only', '--pretty=', commit_ref], capture_output=True, check=True, text=True)
    return {line.strip() for line in result.stdout.splitlines() if line.strip()}


def _read_file_at_ref(ref: str, path: str) -> str:
    result = subprocess.run(['git', 'show', f'{ref}:{path}'], capture_output=True, check=False, text=True)
    if result.returncode != 0:
        raise AssertionError(f'unable_to_read_file_at_ref:{ref}:{path}:{result.stderr.strip()}')
    return result.stdout


def _resolve_commit_ref() -> str:
    result = subprocess.run(['git', 'log', '--format=%H%x09%s'], capture_output=True, check=True, text=True)
    matches: list[str] = []
    for line in result.stdout.splitlines():
        if '\t' not in line:
            continue
        commit_hash, subject = line.split('\t', 1)
        if subject.strip() == PHASE_SUBJECT:
            matches.append(commit_hash)
    for commit_ref in matches:
        if _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_PATHS:
            return commit_ref
    if matches:
        raise AssertionError('phase_456_fix_10_commit_subject_present_but_no_qualifying_implementation_commit')
    raise AssertionError('phase_456_fix_10_commit_not_present_in_local_history')


def test_module_exists_and_exposes_callable_surface() -> None:
    assert MODULE_PATH.exists()
    assert hasattr(nonlinear_module, 'evaluate_recovery_rule')
    assert callable(nonlinear_module.evaluate_recovery_rule)
    assert nonlinear_module.NONLINEAR_CONTROL_CURVE_STATUS == 'implemented'
    assert nonlinear_module.WAGGLE_DANCE_CODENAME == 'Waggle Dance'


def test_candidate_registry_contains_required_ids() -> None:
    assert set(nonlinear_module.CANDIDATE_REGISTRY) == EXPECTED_CANDIDATES
    assert set(nonlinear_module.list_candidate_ids()) == EXPECTED_CANDIDATES


def test_all_runnable_candidates_return_required_metrics() -> None:
    for candidate_id in EXPECTED_CANDIDATES:
        candidate = nonlinear_module.get_candidate_definition(candidate_id)
        outputs = nonlinear_module.evaluate_recovery_rule(candidate_id, candidate.parameters)
        assert set(outputs) == set(nonlinear_module.REQUIRED_OUTPUT_KEYS)
        assert 0.0 <= outputs['organic_ecu_production_rate'] <= 1.0
        assert 0.0 <= outputs['pe_clamp_respect_rate'] <= 1.0
        assert outputs['intervention_duration_epochs'] > 0
        assert outputs['intervention_cost_units'] >= 0


def test_parameter_surface_exists_and_supports_exact_overrides_only() -> None:
    built = nonlinear_module.build_nonlinear_control_parameters(0.58, 1.8, 0.22, 'frontier_first')
    assert tuple(built) == nonlinear_module.NONLINEAR_CONTROL_PARAMETER_KEYS
    leader = nonlinear_module.evaluate_recovery_rule('nonlinear_curve_k58_g18_f22_frontier_first', built)
    assert leader['organic_ecu_production_rate'] == 0.981
    try:
        nonlinear_module.evaluate_recovery_rule(
            'nonlinear_curve_k58_g18_f22_frontier_first',
            nonlinear_module.build_nonlinear_control_parameters(0.59, 1.8, 0.22, 'frontier_first'),
        )
    except ValueError as exc:
        assert 'unsupported_parameter_override:nonlinear_curve_k58_g18_f22_frontier_first:response_knee=0.59' in str(exc)
    else:
        raise AssertionError('unsupported_override_not_rejected')


def test_attestation_doc_contains_required_headings_and_tokens() -> None:
    text = _read(ATTESTATION_PATH)
    for heading in (
        '## 1. Implemented module boundary',
        '## 2. Runnable nonlinear-control candidates',
        '## 3. Weak-field challenger and field boundary',
        '## 4. Mechanism status and codename',
        '## 5. Non-execution and non-authorization statement',
    ):
        body = _section_body(text, heading)
        assert body
        assert len([line for line in body.splitlines() if line.strip()]) >= 2
    for token in (
        'simulations/sim_treasury_scenario5_waggle_dance_recovery_rule.py',
        'waggle_dance_codename=Waggle Dance',
        'nonlinear_control_curve',
        'nonlinear_control_curve_status=implemented',
        'nonlinear_curve_k56_g18_f22_frontier_first',
        'nonlinear_curve_k58_g18_f22_frontier_first',
        'nonlinear_curve_k60_g18_f22_frontier_first',
        'mixed_queue_and_production',
        'response_knee',
        'control_gain',
        'release_floor',
        'bias',
        'Legacy production-band carry-forwards are not automatically included in the nonlinear-control execution field.',
        'No Scenario-5 execution occurs in Phase 456 Fix 10.',
        'No CDL-050 opening or ratification occurs in Phase 456 Fix 10.',
        'Fix 11 may freeze nonlinear-control candidates because nonlinear_control_curve_status=implemented.',
    ):
        assert token in text


def test_walkthrough_and_status_point_to_fix_11() -> None:
    walkthrough = _read(WALKTHROUGH_PATH)
    status = _read(STATUS_PATH)
    assert 'waggle_dance_codename=Waggle Dance' in walkthrough
    assert 'nonlinear_control_curve_status=implemented' in walkthrough
    assert 'No Scenario-5 execution was run in Phase 456 Fix 10.' in walkthrough
    assert 'No `ilc_core/` files were changed.' in walkthrough
    assert 'CDL-050 remains unopened.' in walkthrough
    assert 'Phase 456 Fix 11 — nonlinear-control commission brief freeze' in walkthrough
    assert '## Phase 456 (Fix 10)' in status
    assert 'nonlinear_control_curve_status=implemented' in status
    assert 'Phase 456 Fix 11 — nonlinear-control commission brief freeze' in status


def test_main_commit_touches_expected_paths_only() -> None:
    commit_ref = _resolve_commit_ref()
    assert _changed_paths_for_commit(commit_ref) == EXACT_REQUIRED_PATHS
    assert_head_commit_touched_no_runtime_files(commit_ref)


def test_commit_does_not_open_cdl_050_and_preserves_cdl_051() -> None:
    commit_ref = _resolve_commit_ref()
    before_rows = parse_decision_register_rows(_read_file_at_ref(f'{commit_ref}^', str(DECISION_LOG_PATH)))
    assert before_rows['CDL-051']['status'] == 'ratified'
    assert 'CDL-050' not in before_rows
    for path in EXACT_REQUIRED_PATHS:
        assert FORBIDDEN_TREASURY_TOKEN not in _read_file_at_ref(commit_ref, path)
