from __future__ import annotations

import re
import subprocess
from pathlib import Path

from ilc_core.testing.ratification_mutation_scope_guardrail import (
    assert_head_commit_touched_no_runtime_files,
    parse_decision_register_rows,
)

SEQUENCE_LOCK_PATH = Path('docs/specs/ilc_phase_456_fix_1_recovery_rule_sequence_lock_v0.1.md')
COMMISSION_BRIEF_PATH = Path('docs/specs/ilc_treasury_sim_t_recovery_rule_commission_brief_456_fix_1_v0.1.md')
TEST_PATH = Path('tests/test_phase_456_fix_1_recovery_rule_sequence_lock_and_commission_brief.py')
WALKTHROUGH_PATH = Path(
    'docs/phases/phase_456_fix_1_g8_recovery_rule_carry_forward_sequence_lock_and_commission_brief_walkthrough.md'
)
STATUS_PATH = Path('docs/phases/STATUS.md')
DECISION_LOG_PATH = Path('docs/specs/ilc_constitutional_decision_log_v0.1.md')
PHASE_456_FIX_1_SUBJECT = 'docs(g8): phase 456 fix 1 recovery-rule carry-forward sequence lock and commission brief'
FORBIDDEN_TREASURY_TOKEN = 'ILC_CDL_MUTATION_' + 'AUTHORIZED'
EXACT_REQUIRED_PATHS = {
    str(SEQUENCE_LOCK_PATH),
    str(COMMISSION_BRIEF_PATH),
    str(TEST_PATH),
    str(WALKTHROUGH_PATH),
    str(STATUS_PATH),
}
EXACT_REQUIRED_FILE_PATHS = (
    SEQUENCE_LOCK_PATH,
    COMMISSION_BRIEF_PATH,
    TEST_PATH,
    WALKTHROUGH_PATH,
    STATUS_PATH,
)


def _read(path: Path) -> str:
    return path.read_text(encoding='utf-8')


def _section_body(text: str, heading: str) -> str:
    escaped = re.escape(heading)
    match = re.search(rf'{escaped}\n\n(.*?)(?=\n## |\Z)', text, flags=re.S)
    if not match:
        raise AssertionError(f'section_not_found:{heading}')
    return match.group(1).strip()


def _changed_paths_for_commit(commit_ref: str) -> set[str]:
    result = subprocess.run(
        ['git', 'show', '--name-only', '--pretty=', commit_ref],
        capture_output=True,
        check=True,
        text=True,
    )
    return {line.strip() for line in result.stdout.splitlines() if line.strip()}


def _read_file_at_ref(ref: str, path: str) -> str:
    result = subprocess.run(['git', 'show', f'{ref}:{path}'], capture_output=True, check=False, text=True)
    if result.returncode != 0:
        raise AssertionError(f'unable_to_read_file_at_ref:{ref}:{path}:{result.stderr.strip()}')
    return result.stdout


def _resolve_phase_456_fix_1_commit_ref() -> str:
    result = subprocess.run(['git', 'log', '--format=%H%x09%s'], capture_output=True, check=True, text=True)
    matches: list[str] = []
    for line in result.stdout.splitlines():
        if '\t' not in line:
            continue
        commit_hash, subject = line.split('\t', 1)
        if subject.strip() == PHASE_456_FIX_1_SUBJECT:
            matches.append(commit_hash)

    for commit_ref in matches:
        changed = _changed_paths_for_commit(commit_ref)
        if changed == EXACT_REQUIRED_PATHS:
            return commit_ref

    if matches:
        raise AssertionError('phase_456_fix_1_commit_subject_present_but_no_qualifying_sequence_lock_commit')
    raise AssertionError('phase_456_fix_1_commit_not_present_in_local_history')


def test_sequence_lock_exists_and_contains_required_headings() -> None:
    assert SEQUENCE_LOCK_PATH.exists()
    text = _read(SEQUENCE_LOCK_PATH)
    headings = (
        '## 1. Window identity and non-reopening rule',
        '## 2. Inherited Blocker-1 diagnosis',
        '## 3. Authorized recovery-rule lane',
        '## 4. Scope and non-goals',
        '## 5. Fix-2 required outputs',
        '## 6. Non-authorization statement',
    )
    for heading in headings:
        assert heading in text
        body = _section_body(text, heading)
        assert body
        assert len([line for line in body.splitlines() if line.strip()]) >= 2


def test_sequence_lock_contains_required_non_reopening_and_fix2_contract_tokens() -> None:
    text = _read(SEQUENCE_LOCK_PATH)
    required_tokens = (
        'Phase 456 Fix 1',
        'Phase 456 Fix 2',
        'Window 450-459 remains failed and closed.',
        'CDL-050 remains unopened.',
        'Phases 457-459 remain unauthorized.',
        'Blockers 2, 3, and the L1/L2 prerequisite remain cleared from Phase 456.',
        'organic ECU production rate',
        'P_e clamp-respect rate',
        'Duration and cost do separate inside the family.',
        'Any future CDL-050 reopening still requires a new explicit sequence lock.',
        'blocker_1_fix_2_verdict=',
    )
    for token in required_tokens:
        assert token in text


def test_commission_brief_exists_and_contains_required_headings() -> None:
    assert COMMISSION_BRIEF_PATH.exists()
    text = _read(COMMISSION_BRIEF_PATH)
    headings = (
        '## 1. Recovery-rule subfamily matrix',
        '## 2. Candidate set',
        '## 3. Parameter freeze',
        '## 4. Success criteria',
        '## 5. Discriminating thresholds',
        '## 6. Cross-subfamily reporting rule',
        '## 7. Ambiguous-outcome policy',
    )
    for heading in headings:
        assert heading in text
        body = _section_body(text, heading)
        assert body
        assert len([line for line in body.splitlines() if line.strip()]) >= 2


def test_commission_brief_contains_required_candidate_set_and_rule_tokens() -> None:
    text = _read(COMMISSION_BRIEF_PATH)
    required_tokens = (
        'Subfamily A - epoch-window variants',
        'Subfamily B - clamp-floor variants',
        'production_band_5_epoch',
        'production_band_6_epoch',
        'production_band_7_epoch',
        'production_band_8_epoch',
        'production_band_10_epoch',
        'production_band_5_epoch_with_clamp_floor_low',
        'production_band_5_epoch_with_clamp_floor_high',
        'clamp_floor_low=0.87',
        'clamp_floor_high=0.90',
        'threshold evaluation must be against the best remaining alternative in the full registered candidate set',
        'If exactly one subfamily produces a threshold-clearing leader, that leader is the Fix-2 recommendation without requiring cross-subfamily tie-break.',
        'If neither subfamily produces a threshold-clearing leader, Blocker 1 remains open.',
        'No CDL-050 opening or ratification occurs in Phase 456 Fix 1.',
    )
    for token in required_tokens:
        assert token in text


def test_all_required_deliverables_preserve_non_authorization_boundary_and_forbidden_token_absence() -> None:
    for path in EXACT_REQUIRED_FILE_PATHS:
        assert FORBIDDEN_TREASURY_TOKEN not in _read(path)

    sequence_lock = _read(SEQUENCE_LOCK_PATH)
    commission_brief = _read(COMMISSION_BRIEF_PATH)
    walkthrough = _read(WALKTHROUGH_PATH)
    status = _read(STATUS_PATH)
    assert 'CDL-050 remains unopened.' in sequence_lock
    assert 'No CDL-050 opening or ratification occurs in Phase 456 Fix 1.' in commission_brief
    assert 'CDL-050 remains unopened' in walkthrough
    assert 'CDL-050 remains unopened' in status


def test_walkthrough_and_status_point_to_phase_456_fix_2() -> None:
    walkthrough = _read(WALKTHROUGH_PATH)
    status = _read(STATUS_PATH)
    assert 'Actual result after the main commit:' in walkthrough
    assert 'Phase 456 Fix 2' in walkthrough
    assert 'no simulations were executed' in walkthrough.lower()
    assert '## Phase 456 (Fix 1)' in status
    assert 'Phase 456 Fix 2' in status


def test_phase_456_fix_1_commit_touches_expected_paths_only() -> None:
    commit_ref = _resolve_phase_456_fix_1_commit_ref()
    changed = _changed_paths_for_commit(commit_ref)
    assert changed == EXACT_REQUIRED_PATHS
    assert_head_commit_touched_no_runtime_files(commit_ref=commit_ref)


def test_phase_456_fix_1_commit_does_not_open_cdl_050_and_preserves_cdl_051() -> None:
    commit_ref = _resolve_phase_456_fix_1_commit_ref()
    before_rows = parse_decision_register_rows(_read_file_at_ref(f'{commit_ref}^1', str(DECISION_LOG_PATH)))
    after_rows = parse_decision_register_rows(_read_file_at_ref(commit_ref, str(DECISION_LOG_PATH)))
    assert before_rows == after_rows
    assert before_rows['CDL-051']['status'] == 'ratified'
    assert 'CDL-050' not in before_rows
    assert 'CDL-050' not in after_rows
