...
def test_window_1273_1280_planning_index_points_to_open_sequence_lock() -> None:
    ...
    assert "Window 1273-1280 OPEN through Phase 1277" in text
    assert "window_1273_1280_sequence_lock_committed" in text
    assert "transport_principal_public_path_adr_runtime_preflight_phase_1277.v0.1" in text
    assert "transport_principal_public_p2p_not_activated_phase_1277" in text
    assert "requester_id_fallback_still_forbidden_phase_1277" in text
    assert "non_loopback_projection_still_blocked_phase_1277" in text
    assert "public_fetch_serving_not_enabled_phase_1277" in text
    assert "Phase 1278 is next and remains SENSITIVE" in text
    assert "transport_principal_public_path_adr_runtime_preflight_phase_1277.v0.1" in text
    assert "sidecar_non_loopback_projection_authorization_preflight_phase_1278.v0.1" in text
...
def test_window_1273_1280_outside_audit_records_hardening_without_authority_expansion() -> None:
    ...
    assert "Window 1273-1280 OPEN through Phase 1277" in index
