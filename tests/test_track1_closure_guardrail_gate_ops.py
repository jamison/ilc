...
NON_REPLAY_DOMAIN_EXCEPTION_GATE_SCRIPT = Path("tools/check_non_replay_domain_exception_migration_guardrails.sh")
...
def test_ops_gate_scripts_include_track1_guardrail_prestep() -> None:
    ...
    assert NON_REPLAY_DOMAIN_EXCEPTION_GATE_SCRIPT.name in ci_text
    assert "=== CI Gate Step -3: Non-Replay Domain Exception Migration Guardrails ===" in ci_text
    ...
    assert NON_REPLAY_DOMAIN_EXCEPTION_GATE_SCRIPT.name in release_text
    assert "=== Release Gate Step -3: Non-Replay Domain Exception Migration Guardrails ===" in release_text
...
def test_ci_gate_dry_run_lists_deterministic_command_plan() -> None:
    ...
    assert "check_non_replay_domain_exception_migration_guardrails.sh" in result.stdout
