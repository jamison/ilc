...
@pytest.mark.parametrize("creator_id", ["", " ", "legacy", "A" * 96, "a" * 95, "a" * 97])
...
@pytest.mark.parametrize("refuting_agent_id", ["", " ", "legacy", "B" * 96, "b" * 95, "b" * 97])
...
@pytest.mark.parametrize("member_id", ["", " ", " member ", "legacy", "C" * 96, "c" * 95])
...
@pytest.mark.parametrize("stake", [0.5, Decimal("NaN"), Decimal("Infinity"), Decimal("-0.1")])
def test_coauthorship_rejects_invalid_member_stake(stake: object) -> None:
    with pytest.raises(ValueError) as exc_info:
        settle_attribution_batch(
            _sealed_batch(_coauthor_event()),
    ...
    assert str(exc_info.value) in {
        "stake_map_member_stake_must_be_decimal",
        "stake_map_member_stake_must_be_non_negative_finite_decimal",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((("not-cidv1", VALID_AGENT_ID),), "provenance_chain_node_id_must_be_non_empty_string"),
        (((VALID_NODE_ID, "legacy"),), "provenance_chain_creator_id_must_be_non_empty_string"),
...
def test_passive_ecu_rejects_malformed_star_node_id_when_present() -> None:
    ...
    assert str(exc_info.value) == "passive_ecu_node_id_must_be_non_empty_string"
